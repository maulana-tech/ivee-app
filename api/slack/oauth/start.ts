...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!ent || ent.features.tier < 1) {
    return new Response(JSON.stringify({ error: 'pro_required', message: 'Slack notifications are available on the Pro plan.', upgradeUrl: 'https://ivee.app/pro' }), { status: 403, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
  }
