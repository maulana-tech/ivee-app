...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!ent || ent.features.tier < 1) {
    return jsonResponse({ error: 'pro_required', message: 'Event publishing is available on the Pro plan.', upgradeUrl: 'https://ivee.app/pro' }, 403, cors);
  }
  ...
  const res = await fetch(
    ...
    { method: 'POST', headers: { Authorization: `Bearer ${upstashToken}`, 'User-Agent': 'ivee-edge/1.0' } },
  );
