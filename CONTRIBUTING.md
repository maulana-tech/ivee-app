...
2. **Clone** your fork locally:
   ```bash
   git clone https://github.com/<your-username>/ivee.git
   cd ivee
   ```
...
- Sebuf handler implementations go in `server/ivee/{domain}/v1/`
...
- Proto definitions go in `proto/ivee/{domain}/v1/`
...
1. **Proto definitions** in `proto/ivee/{domain}/v1/` define services and messages
...
3. **Handlers** in `server/ivee/{domain}/v1/handler.ts` implement the service interface
...
2. **Add the proto service** (if the data needs a backend proxy) — define messages and RPC methods in `proto/ivee/{domain}/v1/`
3. **Generate stubs** — run `make generate`
4. **Implement the handler** in `server/ivee/{domain}/v1/`
...
rclone copy public/data/country-boundary-overrides.geojson r2:ivee-maps/
```
...
Use the [Bug Report issue template](https://github.com/koala73/ivee/issues/new/choose) when available.
...
Use the [Feature Request issue template](https://github.com/koala73/ivee/issues/new/choose) when available.
