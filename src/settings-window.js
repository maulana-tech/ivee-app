...
function getLocalizedPanelName(panelKey, fallback) {
...
export function initSettingsWindow() {
    ...
    const panelSettings = loadFromStorage(STORAGE_KEYS.panels, DEFAULT_PANELS);
    ...
    for (const key of Object.keys(panelSettings)) {
        if (!validPanelKeys.has(key) && key !== 'runtime-config')
    ...
    function render() {
        const panelEntries = Object.entries(panelSettings).filter(([key]) => (key !== 'runtime-config' || isDesktopApp) && (!key.startsWith('cw-') || isProUser()));
        ...
        if (grid) {
            ...
            grid.querySelectorAll('.panel-toggle-item').forEach((item) => {
                item.addEventListener('click', () => {
                    ...
                    if (config) {
                        if (!config.enabled && !isPanelEntitled(panelKey, ALL_PANELS[panelKey] ?? config, isProUser()))
                            return;
