...
export class MonitorPanel extends Panel {
    constructor(initialMonitors = []) {
        ...
        Object.defineProperty(this, "onMonitorsChange", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    renderInput() {
        ...
        const inputContainer = h('div', { className: 'monitor-input-container' }, input, h('button', { className: 'monitor-add-btn', id: 'addMonitorBtn', onClick: () => this.addMonitor() }, t('components.monitor.add')));
    ...
    renderMonitorsList() {
        ...
        if (!list)
            return;
        replaceChildren(list, ...this.monitors.map((m) => h('span', { className: 'monitor-tag' }, h('span', { className: 'monitor-tag-color', style: { background: m.color } }), m.keywords.join(', '), h('span', {
    ...
    renderResults(news) {
        ...
        if (this.monitors.length === 0) {
            replaceChildren(results, h('div', { style: 'color: var(--text-dim); font-size: 10px; margin-top: 12px;' }, t('components.monitor.addKeywords')));
            return;
        }
        const matchedItems = [];
        news.forEach((item) => {
        ...
        if (unique.length === 0) {
            replaceChildren(results, h('div', { style: 'color: var(--text-dim); font-size: 10px; margin-top: 12px;' }, t('components.monitor.noMatches', { count: String(news.length) })));
            return;
        }
        ...
        replaceChildren(results, h('div', { style: 'color: var(--text-dim); font-size: 10px; margin: 12px 0 8px;' }, countText), ...unique.slice(0, 10).map((item) => h('div', {
            className: 'item',
            style: `border-left: 2px solid ${item.monitorColor || ''}; padding-left: 8px; margin-left: -8px;`,
        }, h('div', { className: 'item-source' }, item.source), h('a', {
            className: 'item-title',
            href: sanitizeUrl(item.link),
            target: '_blank',
            rel: 'noopener',
        }, item.title), h('div', { className: 'item-time' }, formatTime(item.pubDate)))));
    }
    onChanged(callback) {
