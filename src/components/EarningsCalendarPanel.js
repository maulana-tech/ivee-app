...
async function getMarketClient() {
    if (!_client) {
        ...
        _client = new MarketServiceClient(getRpcBaseUrl(), { fetch: (...args) => globalThis.fetch(...args) });
    }
    return _client;
}
function fmtEps(v) {
...
function renderGroup(date, entries, isFirst) {
    ...
    async refreshFromRpc() {
        try {
            ...
            if (resp.unavailable || !resp.earnings?.length) {
                if (!this._hasData)
                    this.showError('No earnings data', () => void this.fetchData());
                return false;
            }
            this.render(resp.earnings);
            return true;
        }
        catch (e) {
            if (!this._hasData)
