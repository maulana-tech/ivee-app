...
export class GroceryBasketPanel extends Panel {
    ...
    renderBasket(data) {
        ...
        const headerCells = countries.map(c => `<th class="gb-country-header" title="${escapeHtml(c.name)}">${escapeHtml(c.flag)}<br><span class="gb-country-name">${escapeHtml(c.name)}</span></th>`).join('');
