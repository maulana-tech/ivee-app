...
export class UcdpEventsPanel extends Panel {
    constructor() {
        ...
        Object.defineProperty(this, "onEventClick", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        this.content.addEventListener('click', (e) => {
            ...
            const row = e.target.closest('.ucdp-row');
            if (row) {
    ...
    renderContent() {
        ...
        const tabsHtml = tabs.map(t => `<button class="panel-tab ${t.key === this.activeTab ? 'active' : ''}" data-tab="${t.key}">${t.label} <span class="ucdp-tab-count">${tabCounts[t.key]}</span></button>`).join('');
