...
export class DeductionPanel extends Panel {
    constructor(getLatestNews) {
        ...
        Object.defineProperty(this, "geoInputEl", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        this.formEl = h('form', { className: 'deduction-form' }, this.inputEl, formRow);
        this.formEl.addEventListener('submit', this.handleSubmit.bind(this));
        this.resultContainer = h('div', { className: 'deduction-result' });
        const container = h('div', { className: 'deduction-panel-content' }, this.formEl, this.resultContainer);
    ...
    reformatResult(container) {
        ...
        for (const node of nodes) {
            ...
            if (current)
                ...
                groups.push({ cls: '', label: '', nodes: [node] }); // unsectioned
        }
        if (groups.every(g => !g.cls))
            return; // nothing to restructure
        ...
        for (const group of groups) {
            ...
            if (bodyNodes.length === 0 && group.nodes[0]) {
                ...
                const clone = group.nodes[0].cloneNode(true);
