...
import { fetchHotspotContext, formatArticleDate, extractDomain } from '@/services/gdelt-intel';
...
function formatPositionSource(source) {
    if (source === 'POSITION_SOURCE_WINGBITS') {
        return '<a href="https://wingbits.com?utm_source=ivee&utm_medium=referral&utm_campaign=ivee" target="_blank" rel="noopener" style="color:inherit">wingbits.com</a>';
    }
...
export class MapPopup {
    constructor(container) {
        ...
        Object.defineProperty(this, "chokepointData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "transitChart", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "isMobileSheet", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        });
        Object.defineProperty(this, "sheetTouchStartY", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "sheetCurrentOffset", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "mobileDismissThreshold", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 96
        });
        Object.defineProperty(this, "outsideListenerTimeoutId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "handleEscapeKey", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: (e) => {
        ...
        Object.defineProperty(this, "handleSheetTouchStart", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: (e) => {
        ...
        Object.defineProperty(this, "handleSheetTouchMove", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: (e) => {
        ...
        Object.defineProperty(this, "handleSheetTouchEnd", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: () => {
    ...
    show(data) {
        ...
        if (data.type === 'waterway') {
            const waterway = data.data;
            const cp = this.chokepointData?.chokepoints?.find(c => c.name.toLowerCase() === waterway.name.toLowerCase());
            const chartEl = this.popup.querySelector('[data-transit-chart]');
        ...
        this.popup.addEventListener('click', (e) => {
            ...
            const toggle = target.closest('.cluster-toggle');
            if (toggle) {
    ...
    clampPopupToViewport() {
        if (!this.popup || this.isMobileSheet)
            return;
    ...
    renderContent(data) {
        switch (data.type) {
            ...
            case 'aircraft':
                return this.renderAircraftPopup(data.data);
            case 'militaryFlight':
            ...
            case 'gpsJamming':
                return this.renderGpsJammingPopup(data.data);
            case 'radiation':
                return this.renderRadiationPopup(data.data);
            default:
                return '';
        }
    }
    renderRadiationPopup(observation) {
    ...
    renderHotspotPopup(hotspot, relatedNews) {
        ...
        const trendColors = { 'escalating': getCSSColor('--semantic-critical'), 'stable': getCSSColor('--semantic-elevated'), 'de-escalating': getCSSColor('--semantic-normal') };
    ...
    async loadWingbitsLiveFlight(hexCode) {
        if (!this.popup)
            return;
        ...
        try {
            ...
            if (!this.popup || !section.isConnected)
                return;
            if (!live) {
                section.innerHTML = '';
                return;
          ...
          <div class="flight-times" style="font-size:11px;display:grid;grid-template-columns:1fr auto 1fr;gap:2px 8px;margin-bottom:6px;opacity:0.85">
            ...
            if (live.registration)
                rows.push(`<div class="popup-stat"><span class="stat-label">Reg</span><span class="stat-value">${escapeHtml(live.registration)}</span></div>`);
            if (live.model)
                rows.push(`<div class="popup-stat"><span class="stat-label">Model</span><span class="stat-value">${escapeHtml(live.model)}</span></div>`);
            if (live.operator)
                rows.push(`<div class="popup-stat"><span class="stat-label">Operator</span><span class="stat-value">${escapeHtml(live.operator)}</span></div>`);
            if (live.verticalRate !== 0)
                rows.push(`<div class="popup-stat"><span class="stat-label">Climb</span><span class="stat-value">${live.verticalRate > 0 ? '+' : ''}${Math.round(live.verticalRate)} fpm</span></div>`);
    ...
    getTimeAgo(date) {
        ...
        if (seconds < 60)
            return t('popups.timeAgo.s', { count: seconds });
        const minutes = Math.floor(seconds / 60);
        if (minutes < 60)
            return t('popups.timeAgo.m', { count: minutes });
        const hours = Math.floor(minutes / 60);
        if (hours < 24)
            return t('popups.timeAgo.h', { count: hours });
    ...
    getTimeUntil(date) {
        ...
        if (ms <= 0)
            return t('popups.expired');
        const hours = Math.floor(ms / (1000 * 60 * 60));
        if (hours < 1)
            return `${Math.floor(ms / (1000 * 60))}${t('popups.timeUnits.m')}`;
        if (hours < 24)
            return `${hours}${t('popups.timeUnits.h')}`;
...
${isFeatureAvailable('wingbitsEnrichment') ? '<div class="wingbits-live-section"><div class="wingbits-live-loading" style="font-size:11px;opacity:0.5;padding:4px 0">Loading Wingbits live data…</div></div>' : ''}
    ...
    sanitizeClassToken(value, fallback = 'unknown') {
...
Object.defineProperty(MapPopup, "OPERATOR_COUNTRY_MAP", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
...
function formatRadiationSources(observation) {
...
function formatRadiationConfidence(confidence) {
    switch (confidence) {
        case 'high':
            return 'High';
        case 'medium':
