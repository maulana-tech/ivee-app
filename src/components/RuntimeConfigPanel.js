...
import { RUNTIME_FEATURES, getEffectiveSecrets, getRuntimeConfigSnapshot, getSecretState, isFeatureAvailable, isFeatureEnabled, setFeatureToggle, setSecretValue, subscribeRuntimeConfig, validateSecret, verifySecretWithApi, } from '@/services/runtime-config';
...
export class RuntimeConfigPanel extends Panel {
    constructor(options = {}) {
        ...
        Object.defineProperty(this, "featureFilter", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "hiddenByUser", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
    ...
    setEffectiveVisibility(visible) {
        if (visible)
            super.show();
        else
            super.hide();
    ...
    renderFeature(feature) {
        ...
        const allStaged = !available && effectiveSecrets.every((k) => getSecretState(k).valid || (this.pendingSecrets.has(k) && this.validatedKeys.get(k) !== false));
    ...
    attachListeners() {
        ...
        if (!isDesktopRuntime())
            return;
        if (this.mode === 'alert') {
            this.content.querySelector('[data-early-access]')?.addEventListener('click', () => {
                const url = 'https://www.ivee.app/pro';
                if (isDesktopRuntime()) {
                    void invokeTauri('open_url', { url }).catch(() => window.open(url, '_blank'));
                }
                else {
    ...
    async fetchOllamaModels(select) {
        ...
        try {
            ...
            select.innerHTML = models.map(name => `<option value="${escapeHtml(name)}" ${name === currentModel ? 'selected' : ''}>${escapeHtml(name)}</option>`).join('');
