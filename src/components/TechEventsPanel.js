...
export class TechEventsPanel extends Panel {
    ...
    async fetchEvents() {
        ...
        if (hydrated?.events?.length) {
            ...
            this.setCount(hydrated.conferenceCount || hydrated.events.filter((e) => e.type === 'conference').length);
            this.loading = false;
            this.render();
            return;
        }
    ...
    render() {
        if (this.loading) {
            replaceChildren(this.content, h('div', { className: 'tech-events-loading' }, h('div', { className: 'loading-spinner' }), h('span', null, t('components.techEvents.loading'))));
            return;
        }
        if (this.error) {
        ...
        replaceChildren(this.content, h('div', { className: 'tech-events-panel' }, h('div', { className: 'panel-tabs' }, ...tabEntries.map(([view, label]) => h('button', {
            className: `panel-tab ${this.viewMode === view ? 'active' : ''}`,
            dataset: { view },
            onClick: () => { this.viewMode = view; this.render(); },
        }, label))), h('div', { className: 'tech-events-stats' }, h('span', { className: 'stat' }, `📅 ${t('components.techEvents.conferencesCount', { count: String(upcomingConferences.length) })}`), h('span', { className: 'stat' }, `📍 ${t('components.techEvents.onMap', { count: String(mappableCount) })}`), h('a', { href: 'https://www.techmeme.com/events', target: '_blank', rel: 'noopener', className: 'source-link' }, t('components.techEvents.techmemeEvents'))), h('div', { className: 'tech-events-list' }, ...(filteredEvents.length > 0
    ...
    buildEvent(event) {
        ...
        return h('div', { className }, h('div', { className: 'event-date' }, h('span', { className: 'event-month' }, startDate.toLocaleDateString('en-US', { month: 'short' }).toUpperCase()), h('span', { className: 'event-day' }, String(startDate.getDate())), isToday ? h('span', { className: 'today-badge' }, t('components.techEvents.today')) : false, isSoon ? h('span', { className: 'soon-badge' }, t('components.techEvents.soon')) : false), h('div', { className: 'event-content' }, h('div', { className: 'event-header' }, h('span', { className: 'event-icon' }, typeIcons[event.type] ?? '📌'), h('span', { className: 'event-title' }, event.title), safeEventUrl
            ? h('a', { href: safeEventUrl, target: '_blank', rel: 'noopener', className: 'event-url', title: t('components.techEvents.moreInfo') }, '↗')
            : false), h('div', { className: 'event-meta' }, h('span', { className: 'event-dates' }, `${dateStr}${endDateStr}`), event.location
            ? h('span', { className: 'event-location' }, event.location)
            : false, isDesktopRuntime() ? h('button', {
        ...
        }, '\u{1F9E0}') : false, event.coords && !event.coords.virtual
            ? h('button', {
