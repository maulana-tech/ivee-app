...
function resolveIata(token) {
    ...
    if (cityMatch)
        return cityMatch.iata;
    if (EXTRA_CITY_IATA[low])
        return EXTRA_CITY_IATA[low];
    // Whole-word name match — only if the token uniquely identifies one airport
    const nameMatches = MONITORED_AIRPORTS.filter(a => a.name.toLowerCase().split(/[\s\-–./]+/).includes(low));
    if (nameMatches.length === 1)
        return nameMatches[0].iata;
    return undefined;
}
function extractAirports(words) {
    const result = [];
    const seen = new Set();
    let i = 0;
    while (i < words.length) {
        ...
        if (iata && !seen.has(iata)) {
            ...
            seen.add(iata); // consume the token even if already seen
        i++;
    }
    return result;
}
...
function parseIntent(raw) {
    ...
    if (/^TRACK\s/.test(q)) {
        ...
        if (token)
            return { type: 'TRACK', callsign: token };
    }
...
async function executeIntent(intent) {
    ...
    if (intent.type === 'FLIGHT_STATUS') {
        ...
        if (!flights.length)
            return { html: `<div class="cmd-empty">No results for ${escapeHtml(intent.flightNumber)}.</div>` };
        const f = flights[0];
    ...
    if (intent.type === 'PRICE_WATCH') {
        ...
        const dateChips = [1, 3, 7, 14, 30].map(days => {
...
export class AviationCommandBar {
      ...
      <div id="aviation-cmd-box">
        ...
        this.overlay.addEventListener('click', (e) => {
            ...
            if (rerunBtn?.dataset['rerun']) {
                ...
                const inp = this.overlay?.querySelector('#aviation-cmd-input');
                if (inp && cmd) {
                    inp.value = cmd;
        ...
        const input = this.overlay.querySelector('#aviation-cmd-input');
    ...
    renderHistory() {
        ...
        el.innerHTML = `<div style="font-size:11px;color:#6b7280;margin-top:4px">${h.map(c => `<button class="cmd-hist-btn" style="background:none;border:none;color:#9ca3af;cursor:pointer;font-size:11px;padding:1px 4px;border-radius:2px">${escapeHtml(c)}</button>`).join('')}</div>`;
    ...
    updateSuggestions(val) {
        ...
        el.innerHTML = suggestions.slice(0, 4).map(s => `<button class="cmd-sug-btn" style="background:none;border:1px solid var(--border,#2a2a2a);border-radius:3px;color:var(--text-dim,#9ca3af);cursor:pointer;font-size:11px;padding:2px 6px;margin:2px">${escapeHtml(s)}</button>`).join('');
    ...
    addStyles() {
        if (document.getElementById('aviation-cmd-styles'))
            return;
