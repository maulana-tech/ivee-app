...
import { fetchConsumerPriceOverview, fetchConsumerPriceCategories, fetchConsumerPriceMovers, fetchRetailerPriceSpreads, fetchConsumerPriceFreshness, fetchAllMarketsOverview, MARKETS, SINGLE_MARKETS, DEFAULT_MARKET, DEFAULT_BASKET, } from '@/services/consumer-prices';
...
function pctBadge(val, invertColor = false) {
    if (val == null || val === 0)
        return '<span class="cp-badge cp-badge--neutral">—</span>';
...
function pricePressureBadge(wowPct) {
    if (Math.abs(wowPct) < 0.5)
        return '<span class="cp-pressure cp-pressure--steady">Stable</span>';
    if (wowPct >= 2)
        return '<span class="cp-pressure cp-pressure--stress">Rising</span>';
    if (wowPct > 0.5)
        return '<span class="cp-pressure cp-pressure--watch">Mild Rise</span>';
    return '<span class="cp-pressure cp-pressure--green">Easing</span>';
}
function freshnessLabel(min) {
    if (min == null || min === 0)
        return 'Unknown';
    if (min < 60)
        return `${min}m ago`;
...
function freshnessClass(min) {
    if (min == null)
        return 'cp-fresh--unknown';
    if (min <= 60)
        return 'cp-fresh--ok';
    if (min <= 240)
        return 'cp-fresh--warn';
...
export class ConsumerPricesPanel extends Panel {
    constructor() {
        ...
        Object.defineProperty(this, "movers", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "freshness", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "allMarkets", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        Object.defineProperty(this, "loading", {
            ...
            writable: true,
            value: false
        }); // tracks in-flight fetch to avoid duplicates
        this.content.addEventListener('click', (e) => this.handleClick(e));
    }
    handleClick(e) {
        const target = e.target;
        const marketBtn = target.closest('[data-market]');
        ...
        if (tab?.dataset.tab) {
            this.settings.tab = tab.dataset.tab;
            saveSettings(this.settings);
            this.render();
            return;
        }
        const catRow = target.closest('[data-category]');
        ...
        if (rangeBtn?.dataset.range) {
            this.settings.range = rangeBtn.dataset.range;
            saveSettings(this.settings);
            void this.fetchData();
            return;
        }
    ...
    renderCategories() {
        ...
        if (!cats?.length)
            return this.renderEmptyState('No category data yet');
        return `
    ...
    renderMovers() {
        const d = this.movers;
        if (!d)
            return this.renderEmptyState('No price movement data yet');
        const { categoryFilter } = this.settings;
        const filterFn = (m) => !categoryFilter || m.category === categoryFilter;
    ...
    renderSpreadRow(r, rank, currency) {
    ...
    renderHealth() {
        ...
        if (!d?.retailers?.length)
            return this.renderEmptyState('Health data not yet available');
        return `
