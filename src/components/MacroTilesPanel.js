...
function cpiYoY(obs) {
    if (obs.length < 13)
        return { value: null, prior: null, date: '' };
...
function deltaColor(delta, lowerIsBetter, neutral) {
    if (neutral)
        return 'var(--text-dim)';
    if (delta === 0)
        return 'var(--text-dim)';
...
export class MacroTilesPanel extends Panel {
    constructor() {
        ...
        Object.defineProperty(this, "_usTiles", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "_eurostat", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "_estrObs", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
    ...
    async fetchData() {
        ...
        try {
            ...
            if (!this._usTiles.some(t => t.value !== null)) {
                if (!this._hasData)
                    this.showError('Macro data unavailable', () => void this.fetchData());
                return false;
            }
            this._hasData = true;
            this._render();
