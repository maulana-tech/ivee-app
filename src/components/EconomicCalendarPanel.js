...
function countdown(dateStr) {
    ...
    if (days === 1)
        return 'tomorrow';
    if (days < 0)
        return Math.abs(days) < 14 ? `${Math.abs(days)}d ago` : `${Math.round(Math.abs(days) / 7)}w ago`;
    if (days < 14)
        return `in ${days}d`;
...
export class EconomicCalendarPanel extends Panel {
    ...
    _filterEvents() {
        if (this._region === 'us')
            return this._events.filter((e) => e.country === 'US');
        if (this._region === 'eu')
            return this._events.filter((e) => EU_COUNTRIES.has(e.country));
        return this._events;
    }
    _renderRegionTabs() {
        const tabs = [
