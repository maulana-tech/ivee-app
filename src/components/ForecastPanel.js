...
const DOMAINS = ['all', 'conflict', 'market', 'supply_chain', 'political', 'military', 'cyber', 'infrastructure'];
...
const PATH_ID_LABELS = {
    escalation: 'Escalation',
    containment: 'Containment',
    market_cascade: 'Market Cascade',
};
function parseTheaters(json) {
    try {
        const arr = JSON.parse(json);
        if (!Array.isArray(arr))
            return [];
        return arr.filter((v) => v && typeof v === 'object' && typeof v.theaterId === 'string' && typeof v.theaterLabel === 'string');
    }
    catch {
        return [];
    }
...
export class ForecastPanel extends Panel {
    constructor() {
        ...
        Object.defineProperty(this, "activeDomain", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 'all'
        });
        Object.defineProperty(this, "theaters", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "expandedTheaterId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        this.content.addEventListener('click', (e) => {
            ...
            const filterBtn = target.closest('[data-fc-domain]');
            if (filterBtn) {
                this.activeDomain = filterBtn.dataset.fcDomain || 'all';
                this.render();
                return;
            }
            const theaterBtn = target.closest('[data-fc-theater]');
            if (theaterBtn) {
            ...
            const toggle = target.closest('[data-fc-toggle]');
            if (toggle) {
                const item = toggle.closest('.fc-prob-item');
                const panelId = toggle.dataset.fcToggle;
                const detail = panelId ? item?.querySelector(`[data-fc-panel="${panelId}"]`) : null;
                if (detail)
            ...
            const probRow = target.closest('.fc-prob-row');
            if (probRow) {
                const item = probRow.closest('.fc-prob-item');
                const toggleRow = item?.querySelector('.fc-toggle-row');
                if (toggleRow)
                    toggleRow.style.display = toggleRow.style.display === 'flex' ? '' : 'flex';
                return;
            }
        });
    }
    ...
    updateSimulation(theaterSummariesJson) {
    ...
    render() {
        ...
        const filtersHtml = DOMAINS.map(d => `<button class="fc-filter${d === this.activeDomain ? ' fc-active' : ''}" data-fc-domain="${d}">${DOMAIN_LABELS[d]}</button>`).join('');
    ...
    renderTheaterDetail(t) {
      ...
      `;
        ...
        const reactions = t.dominantReactions.map(r => `<div class="fc-footer-item fc-react-item">${escapeHtml(r)}</div>`).join('');
        const stabilizers = t.stabilizers.map(s => `<div class="fc-footer-item fc-stab-item">${escapeHtml(s)}</div>`).join('');
        const invalidators = t.invalidators.map(s => `<div class="fc-footer-item fc-inval-item">${escapeHtml(s)}</div>`).join('');
        return `
    ...
    renderProbRow(f) {
        ...
        const signalsHtml = sigs.length > 0
            ? sigs.map(s => `<div class="fc-signal">${escapeHtml(s.value.replace(/^[\s\u2013\u2014\-]+/, ''))}</div>`).join('')
            : '';
