...
export class DefensePatentsPanel extends Panel {
    constructor() {
        ...
        Object.defineProperty(this, "patents", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
    ...
    render() {
        if (this.loading) {
            replaceChildren(this.content, h('div', { className: 'defense-patents-loading' }, h('div', { className: 'loading-spinner' }), h('span', null, 'Loading R&D filings…')));
            return;
        }
        if (this.error) {
        ...
        const tabs = [
            ['all', 'All'],
            ...Object.entries(CPC_LABELS).map(([code, label]) => [code, label]),
        ];
        const filtered = this.getFiltered();
        replaceChildren(this.content, h('div', { className: 'defense-patents-panel' }, h('div', { className: 'panel-tabs' }, ...tabs.map(([mode, label]) => h('button', {
            className: `panel-tab ${this.viewMode === mode ? 'active' : ''}`,
            onClick: () => { this.viewMode = mode; this.render(); },
        }, label))), h('div', { className: 'defense-patents-list' }, ...(filtered.length > 0
            ? filtered.map(p => this.buildRow(p))
            : [h('div', { className: 'empty-state' }, 'No filings in this category.')]))));
    }
    getFiltered() {
        if (this.viewMode === 'all')
            return this.patents.slice(0, 50);
    ...
    buildRow(p) {
        ...
        return h('div', { className: 'defense-patent-row' }, h('div', { className: 'patent-icon', title: p.cpcDesc || p.cpcCode }, icon), h('div', { className: 'patent-body' }, h('div', { className: 'patent-header' }, h('span', { className: 'patent-assignee' }, p.assignee), safeUrl
            ? h('a', { href: safeUrl, target: '_blank', rel: 'noopener', className: 'patent-link', title: 'View on USPTO' }, '↗')
            : false), h('div', { className: 'patent-title' }, p.title), h('div', { className: 'patent-meta' }, h('span', { className: `patent-cpc cpc-${p.cpcCode}` }, p.cpcDesc || p.cpcCode), date ? h('span', { className: 'patent-date' }, date) : false, p.patentId ? h('span', { className: 'patent-id' }, p.patentId) : false)));
    }
    refresh() {
        void this.fetchPatents();
    }
