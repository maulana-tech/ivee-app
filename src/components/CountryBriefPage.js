...
export class CountryBriefPage {
    constructor() {
        ...
        Object.defineProperty(this, "currentScore", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "currentSignals", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "currentBrief", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "currentHeadlines", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        this.overlay.addEventListener('click', (e) => {
            ...
            const linkShareBtn = target.closest('.cb-link-share-btn');
            if (linkShareBtn) {
            ...
            const exportOption = target.closest('.cb-export-option');
            if (exportOption) {
    ...
    signalChips(signals) {
        ...
        if (signals.criticalNews > 0)
            chips.push(`<span class="signal-chip conflict">🚨 ${signals.criticalNews} Critical News</span>`);
        if (signals.protests > 0)
            chips.push(`<span class="signal-chip protest">📢 ${signals.protests} ${t('modals.countryBrief.signals.protests')}</span>`);
        if (signals.militaryFlights > 0)
            chips.push(`<span class="signal-chip military">✈️ ${signals.militaryFlights} ${t('modals.countryBrief.signals.militaryAir')}</span>`);
        if (signals.militaryVessels > 0)
            chips.push(`<span class="signal-chip military">⚓ ${signals.militaryVessels} ${t('modals.countryBrief.signals.militarySea')}</span>`);
        if (signals.outages > 0)
            chips.push(`<span class="signal-chip outage">🌐 ${signals.outages} ${t('modals.countryBrief.signals.outages')}</span>`);
        if (signals.aisDisruptions > 0)
            chips.push(`<span class="signal-chip outage">🚢 ${signals.aisDisruptions} AIS Disruptions</span>`);
        if (signals.satelliteFires > 0)
            chips.push(`<span class="signal-chip climate">🔥 ${signals.satelliteFires} Satellite Fires</span>`);
        if (signals.radiationAnomalies > 0)
            chips.push(`<span class="signal-chip outage">☢️ ${signals.radiationAnomalies} Radiation Anomalies</span>`);
        if (signals.temporalAnomalies > 0)
            chips.push(`<span class="signal-chip outage">⏱️ ${signals.temporalAnomalies} Temporal Anomalies</span>`);
        if (signals.cyberThreats > 0)
            chips.push(`<span class="signal-chip conflict">🛡️ ${signals.cyberThreats} Cyber Threats</span>`);
        if (signals.earthquakes > 0)
            chips.push(`<span class="signal-chip quake">🌍 ${signals.earthquakes} ${t('modals.countryBrief.signals.earthquakes')}</span>`);
        ...
        if (signals.climateStress > 0)
            chips.push(`<span class="signal-chip climate">🌡️ ${t('modals.countryBrief.signals.climate')}</span>`);
        if (signals.conflictEvents > 0)
            chips.push(`<span class="signal-chip conflict">⚔️ ${signals.conflictEvents} ${t('modals.countryBrief.signals.conflictEvents')}</span>`);
        if (signals.activeStrikes > 0)
            chips.push(`<span class="signal-chip conflict">\u{1F4A5} ${signals.activeStrikes} ${t('modals.countryBrief.signals.activeStrikes')}</span>`);
        ...
        if (signals.orefHistory24h > 0)
            chips.push(`<span class="signal-chip conflict">\u{1F553} ${signals.orefHistory24h} Sirens / 24h</span>`);
        if (signals.aviationDisruptions > 0)
            chips.push(`<span class="signal-chip outage">\u{1F6AB} ${signals.aviationDisruptions} ${t('modals.countryBrief.signals.aviationDisruptions')}</span>`);
        if (signals.gpsJammingHexes > 0)
            chips.push(`<span class="signal-chip outage">\u{1F4E1} ${signals.gpsJammingHexes} ${t('modals.countryBrief.signals.gpsJammingZones')}</span>`);
    ...
    updateInfrastructure(countryCode) {
        ...
        const nearbyPorts = PORTS
            .map((p) => ({ port: p, dist: haversineDistanceKm(centroidLat, centroidLon, p.lat, p.lon) }))
    ...
    timeAgo(date) {
        ...
        if (hours < 1)
            return t('modals.countryBrief.timeAgo.m', { count: Math.floor(ms / 60000) });
        if (hours < 24)
            return t('modals.countryBrief.timeAgo.h', { count: hours });
    ...
    exportBrief(format) {
        ...
        if (format === 'json')
            exportCountryBriefJSON(data);
        else
            exportCountryBriefCSV(data);
    }
    exportPdf() {
...
Object.defineProperty(CountryBriefPage, "BRIEF_BOUNDS", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
...
Object.defineProperty(CountryBriefPage, "INFRA_ICONS", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
...
Object.defineProperty(CountryBriefPage, "INFRA_LABELS", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
