...
function yPos(v, yMin, yMax) {
    const range = yMax - yMin || 1;
    return MT + CH - ((v - yMin) / range) * CH;
}
function buildLine(points, key, yMin, yMax) {
    const coords = points
        .map((p, i) => {
        const v = p[key];
...
function buildLegend() {
    return SERIES.map(s => `<span class="fao-legend-item"><span class="fao-legend-dot" style="background:${s.color}"></span>${escapeHtml(t(`components.faoFoodPriceIndex.${s.key}`))}</span>`).join('');
}
