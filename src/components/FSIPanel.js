...
async function getMarketClient() {
    if (!_marketClient) {
        ...
        _marketClient = new MarketServiceClient(getRpcBaseUrl(), { fetch: (...args) => globalThis.fetch(...args) });
    }
    return _marketClient;
}
let _economicClient = null;
async function getEconomicClient() {
    if (!_economicClient) {
        ...
        const { getRpcBaseUrl } = await import('@/services/rpc-client');
        _economicClient = new EconomicServiceClient(getRpcBaseUrl(), { fetch: (...args) => globalThis.fetch(...args) });
    }
    return _economicClient;
}
function fsiLabelColor(label) {
    if (label === 'Low Stress')
        return '#27ae60';
...
function fsiInterpretation(label) {
    if (label === 'Low Stress')
...
export class FSIPanel extends Panel {
    ...
    async fetchData() {
        ...
        try {
            ...
            if (fsiValue <= 0) {
                if (!this._hasData)
                    this.showError('FSI data unavailable', () => void this.fetchData());
                return false;
            }
            // Fetch EU CISS — check hydrated bootstrap first, then RPC fallback
            let euFsi = null;
            try {
                const hydratedEuFsi = getHydratedData('euFsi');
                if (hydratedEuFsi && !hydratedEuFsi.unavailable && Number.isFinite(hydratedEuFsi.latestValue)) {
                    ...
                    if (!euResp.unavailable && Number.isFinite(euResp.latestValue))
                        euFsi = euResp;
                }
            }
            catch {
