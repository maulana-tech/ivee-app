...
import { getPinnedWebcams, getActiveWebcams, unpinWebcam, toggleWebcam, onPinnedChange, } from '../services/webcams/pinned-store';
const MAX_SLOTS = 4;
const PLAYER_FALLBACK = 'https://webcams.windy.com/webcams/public/embed/player';
function buildPlayerUrl(webcamId, playerUrl) {
    if (playerUrl)
        return playerUrl;
    ...
    render() {
        ...
        if (allPinned.length > MAX_SLOTS) {
            ...
            allPinned.forEach(cam => {
                ...
                if (cam.active)
                    row.classList.add('pinned-webcam-row--active');
