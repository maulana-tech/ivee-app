...
function buildHeadersFromTemplate(template, key) {
...
function extractKeyFromHeaders(headers, template) {
    const colon = template.indexOf(':');
    if (colon === -1)
        return null;
    const headerName = template.slice(0, colon).trim();
    ...
    if (prefix && !actual.startsWith(prefix))
        return null;
...
function extractAuthHint(authNote) {
    const m = authNote.match(/\(([^)]+)\)\s*$/);
    return m?.[1] ?? authNote;
}
export function openMcpConnectModal(options) {
  ...
  `;
    ...
    const apiKeyGroup = modal.querySelector('.mcp-api-key-group');
    const apiKeyInput = modal.querySelector('.mcp-api-key');
    const apiKeyHint = modal.querySelector('.mcp-api-key-hint');
    const toAdvancedBtn = modal.querySelector('.mcp-to-advanced');
    const authHeaderGroup = modal.querySelector('.mcp-auth-header-group');
    const authInput = modal.querySelector('.mcp-auth-header');
    const toSimpleBtn = modal.querySelector('.mcp-to-simple');
    const connectBtn = modal.querySelector('.mcp-connect-btn');
    const connectStatus = modal.querySelector('.mcp-connect-status');
    const toolsSection = modal.querySelector('.mcp-tools-section');
    const toolsList = modal.querySelector('.mcp-tools-list');
    const toolConfig = modal.querySelector('.mcp-tool-config');
    const argsInput = modal.querySelector('.mcp-tool-args');
    const argsError = modal.querySelector('.mcp-args-error');
    const titleInput = modal.querySelector('.mcp-panel-title');
    const refreshInput = modal.querySelector('.mcp-refresh-input');
    const addBtn = modal.querySelector('.mcp-add-btn');
    ...
    function showSimpleMode(preset) {
    ...
    function showAdvancedMode(prefillFromKey = true) {
    ...
    if (!existing) {
        urlInput.addEventListener('input', () => {
            ...
            const presetCards = Array.from(modal.querySelectorAll('.mcp-preset-card'));
            ...
            if (matchedCard) {
                ...
                const fakePreset = { apiKeyHeader: cardApiKeyHeader, authNote: cardAuthNote };
                if (cardApiKeyHeader) {
    ...
    modal.querySelectorAll('.mcp-preset-card').forEach(card => {
    ...
    connectBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (Object.keys(headers).length)
                qs.set('headers', JSON.stringify(headers));
    ...
    addBtn.addEventListener('click', () => {
        ...
        let toolArgs = {};
        try {
            toolArgs = JSON.parse(argsInput.value || '{}');
        }
        catch {
...
function _headersToLine(headers) {
    const entries = Object.entries(headers);
    if (!entries.length)
        return '';
    return entries.map(([k, v]) => `${k}: ${v}`).join('; ');
}
export function closeMcpConnectModal() {
    overlay?.remove();
    overlay = null;
}
