...
function formatDebt(usd) {
    if (!Number.isFinite(usd) || usd <= 0)
        return '$0';
    if (usd >= 1e12)
        return `$${(usd / 1e12).toFixed(1)}T`;
    if (usd >= 1e9)
...
function getCurrentDebt(entry) {
    if (!entry.perSecondRate || !entry.baselineTs)
        return entry.debtUsd ?? 0;
...
export class NationalDebtPanel extends Panel {
    constructor() {
        ...
        Object.defineProperty(this, "filteredEntries", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        Object.defineProperty(this, "visibleCount", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: PAGE_SIZE
        });
        Object.defineProperty(this, "tickerInterval", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "tickerElements", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "lastTickerValues", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        ...
        this.content.addEventListener('click', (e) => {
            ...
            const tab = target.closest('[data-sort]');
            if (tab?.dataset.sort) {
                this.sortMode = tab.dataset.sort;
    ...
    async refresh() {
        if (this.loading)
            return;
        if (Date.now() - this.lastFetch < this.REFRESH_INTERVAL && this.entries.length > 0)
            return;
        this.loading = true;
    ...
    applyFilters() {
        ...
        const base = q
            ? this.entries.filter(e => e.iso3.toLowerCase().includes(q) ||
    ...
    startTicker() {
        ...
        const globalEl = this.content.querySelector('.debt-global-ticker');
        if (globalEl)
            this.tickerElements.set('__global__', globalEl);
        const container = this.content.querySelector('.debt-list');
        if (container) {
            for (const entry of this.filteredEntries.slice(0, this.visibleCount)) {
                const el = container.querySelector(`.debt-ticker[data-iso3="${entry.iso3}"]`);
                if (el)
                    this.tickerElements.set(entry.iso3, el);
            }
        }
