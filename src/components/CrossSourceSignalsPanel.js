...
export class CrossSourceSignalsPanel extends Panel {
    constructor() {
        ...
        Object.defineProperty(this, "evaluatedAt", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "compositeCount", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
    ...
    renderSignal(sig, index) {
        ...
        const contributors = isComposite && sig.contributingTypes.length > 0
            ? `<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:5px">${sig.contributingTypes.slice(0, 5).map(t => `<span style="font-size:9px;font-family:var(--font-mono);padding:1px 5px;background:rgba(255,255,255,0.05);border:1px solid var(--border);color:var(--text-dim);text-transform:uppercase;letter-spacing:0.06em;border-radius:2px">${escapeHtml(t)}</span>`).join('')}</div>`
            : '';
        return `
    ...
    render() {
        if (this.signals.length === 0) {
            if (!this.evaluatedAt) {
                this.showError('Signal aggregator is initializing. First evaluation runs within 15 minutes.', () => { });
            }
            else {
