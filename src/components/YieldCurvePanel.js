...
let _client = null;
async function getEconomicClient() {
    if (!_client) {
...
const SERIES_IDS = ['DGS1MO', 'DGS3MO', 'DGS6MO', 'DGS1', 'DGS2', 'DGS5', 'DGS10', 'DGS30'];
...
function yPos(value, yMin, yMax) {
    const range = yMax - yMin || 1;
    const scale = (value - yMin) / range;
    return MARGIN_T + CHART_H - scale * CHART_H;
}
function buildPolylinePoints(points, yMin, yMax) {
    return points
        .map((p, i) => {
        if (p.value === null)
            return null;
...
function buildYAxisLabels(yMin, yMax) {
    const step = (yMax - yMin) / 3;
    const labels = [];
    for (let i = 0; i <= 3; i++) {
        const val = yMin + step * i;
        const y = yPos(val, yMin, yMax);
        labels.push(`<text x="${(MARGIN_L - 4).toFixed(0)}" y="${y.toFixed(2)}" text-anchor="end" fill="rgba(255,255,255,0.35)" font-size="8" alignment-baseline="middle">${val.toFixed(1)}%</text>`);
        labels.push(`<line x1="${MARGIN_L}" y1="${y.toFixed(2)}" x2="${SVG_W - MARGIN_R}" y2="${y.toFixed(2)}" stroke="rgba(255,255,255,0.06)" stroke-width="1"/>`);
    }
    return labels.join('');
}
function buildXAxisLabels(count) {
...
function buildCircles(points, yMin, yMax, color) {
...
function buildEcbPolyline(ecbRates, yMin, yMax) {
...
function buildEcbCircles(ecbRates, yMin, yMax) {
...
function renderChart(current, prior, ecbRates) {
    const usValues = current.map(p => p.value).filter((v) => v !== null);
    const priorValues = prior.map(p => p.value).filter((v) => v !== null);
...
function miniRateSparkline(obs, color, w = 80, h = 22) {
...
export class YieldCurvePanel extends Panel {
    constructor() {
        ...
        Object.defineProperty(this, "_hasData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        Object.defineProperty(this, "_prior", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "_ecbRates", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "_rateRows", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
    ...
    async fetchData() {
        ...
        try {
            ...
            if (validCount === 0) {
                if (!this._hasData)
                    this.showError('No yield data available', () => void this.fetchData());
                return false;
            }
            this._hasData = true;
            this._render();
            ...
            if (!this._hasData)
                this.showError(e instanceof Error ? e.message : 'Failed to load yield curve', () => void this.fetchData());
            return false;
        }
    }
    _render() {
