...
function storyEscHandler(e) {
    if (e.key === 'Escape')
        closeStoryModal();
}
export function openStoryModal(data) {
    closeStoryModal();
    currentData = data;
...
function downloadStory() {
    ...
    a.download = `ivee-${currentData?.countryCode.toLowerCase() || 'story'}-${Date.now()}.png`;
    a.click();
...
async function shareWhatsApp(data) {
    ...
    const file = new File([currentBlob], `${data.countryCode.toLowerCase()}-ivee.png`, { type: 'image/png' });
