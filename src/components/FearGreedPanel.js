...
function getRegimeState(score) {
    if (score <= 20)
        return { state: 'Crisis / Risk-Off', stance: 'CASH', color: '#c0392b' };
    if (score <= 35)
        return { state: 'Stressed / Defensive', stance: 'DEFENSIVE', color: '#e67e22' };
    if (score <= 50)
        return { state: 'Fragile / Hedged', stance: 'HEDGED', color: '#f1c40f' };
    if (score <= 65)
        return { state: 'Stable / Normal', stance: 'NORMAL', color: '#2ecc71' };
    return { state: 'Strong / Risk-On', stance: 'AGGRESSIVE', color: '#27ae60' };
}
function getDivergenceWarnings(d) {
    const warnings = [];
    ...
    if (mom < 10)
        warnings.push('Momentum at extreme low — broad equity selling pressure');
    if (sent < 15)
        warnings.push('Sentiment in extreme fear zone');
    if (cnn > 0 && Math.abs(comp - cnn) > 20)
        warnings.push(`CNN F&G ${Math.round(cnn)} diverges ${Math.abs(Math.round(comp - cnn))}pts from composite — sentiment/structural disconnect`);
    if (trend < 20)
        warnings.push('Trend in breakdown — price structure deteriorating');
    return warnings;
}
function renderGauge(score, label, delta, color) {
    const cx = 100, cy = 100, R = 88, r = 60;
    function coord(deg, radius) {
    ...
    const segs = zones.map(z => `<path d="M${coord(z.a1, R)} A${R},${R} 0 0,0 ${coord(z.a2, R)} L${coord(z.a2, r)} A${r},${r} 0 0,1 ${coord(z.a1, r)} Z" fill="${z.fill}" opacity="0.88"/>`).join('');
...
const CAT_NAMES = ['sentiment', 'volatility', 'positioning', 'trend', 'breadth', 'momentum', 'liquidity', 'credit', 'macro', 'crossAsset'];
const CAT_DISPLAY = {
    sentiment: 'Sentiment',
...
export class FearGreedPanel extends Panel {
    ...
    renderPanel() {
        ...
        const hdrMetric = (lbl, val) => `<div style="text-align:center;padding:6px 4px">
