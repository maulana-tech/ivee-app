...
function stressFormatRaw(id, raw) {
    if (id === 'ICSA')
        return raw >= 1000 ? (raw / 1000).toFixed(0) + 'K' : raw.toFixed(0);
    if (id === 'VIXCLS')
        return raw.toFixed(2);
    if (id === 'STLFSI4' || id === 'GSCPI')
        return raw.toFixed(3);
    return raw.toFixed(2);
}
const STRESS_NOTIFICATION_KEY = 'wm:economic-stress:last-notified-level';
function notifyIfStressCrossed(score) {
...
function stressComponentCard(c) {
    if (c.missing) {
...
function formatSeriesValue(series) {
    if (series.value === null)
        return 'N/A';
    if (series.unit === '$B')
        return `$${series.value.toLocaleString()}B`;
...
function getSeriesChangeClass(change) {
    if (change === null || change === 0)
        return 'neutral';
    return change > 0 ? 'positive' : 'negative';
}
function getMacroPressure(data) {
    ...
    let score = 0;
    if (vix !== null)
        score += vix >= 25 ? 2 : vix >= 18 ? 1 : 0;
    if (curve !== null)
        score += curve <= 0 ? 2 : curve < 0.5 ? 1 : 0;
    if (unemployment !== null)
        score += unemployment >= 4.5 ? 1 : 0;
    if (fedFunds !== null)
        score += fedFunds >= 5 ? 1 : fedFunds <= 2 ? -1 : 0;
    if (score >= 4) {
        return {
...
export class EconomicPanel extends Panel {
    constructor() {
        ...
        Object.defineProperty(this, "fredData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "blsData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "spendingData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "bisData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "stressData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "fredState", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 'loading'
        });
        Object.defineProperty(this, "fredErrorMsg", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: ''
    ...
    setFredRetrying(remainingSeconds) {
    ...
    updateStress(data) {
        ...
        if (Number.isFinite(data.compositeScore))
            notifyIfStressCrossed(data.compositeScore);
        this.render();
    }
    setLoading(loading) {
        if (loading) {
