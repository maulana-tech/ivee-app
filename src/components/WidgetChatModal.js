...
async function buildWidgetAuthHeaders(isPro) {
...
export function openWidgetChatModal(options) {
    closeWidgetChatModal();
    track('widget-ai-open', { panelId: options.existingSpec?.id });
    const currentTier = options.tier ?? options.existingSpec?.tier ?? 'basic';
  ...
  `;
    ...
    const messagesEl = modal.querySelector('.widget-chat-messages');
    const previewEl = modal.querySelector('.widget-chat-preview');
    const readinessEl = modal.querySelector('.widget-chat-readiness');
    const examplesEl = modal.querySelector('.widget-chat-examples-list');
    const footerStatusEl = modal.querySelector('.widget-chat-footer-status');
    const inputEl = modal.querySelector('.widget-chat-input');
    const sendBtn = modal.querySelector('.widget-chat-send');
    const actionBtn = modal.querySelector('.widget-chat-action-btn');
    const closeBtn = modal.querySelector('.modal-close');
    const sessionHistory = [...(options.existingSpec?.conversationHistory ?? [])];
    let currentSessionHtml = options.existingSpec?.html ?? null;
    let requestInFlight = false;
    let preflightReady = false;
    let pendingSaveSpec = null;
    if (options.initialMessage)
        inputEl.value = options.initialMessage;
    ...
    async function runPreflight() {
        ...
        try {
            ...
            if (!currentSessionHtml)
                renderPreviewState(previewEl, 'ready_to_prompt');
    ...
    const submit = async () => {
        ...
        const timeoutMs = isPro ? 120000 : 60000;
            ...
            while (true) {
                ...
                for (const line of lines) {
                    ...
                    if (event.type === 'tool_call') {
                        ...
                        sessionHistory.push({ role: 'user', content: prompt }, { role: 'assistant', content: assistantSummary });
...
export function closeWidgetChatModal() {
    if (abortController) {
        abortController.abort();
    ...
    if (overlay) {
        const o = overlay;
        if (o._escHandler)
            document.removeEventListener('keydown', o._escHandler);
        overlay.remove();
        overlay = null;
    }
}
function renderExampleChips(container, inputEl, isPro) {
    container.innerHTML = '';
...
function resolvePreflightMessage(status, payload, isPro) {
    if (status === 403)
    ...
    if (payload?.anthropicConfigured === false)
        return t('widgets.preflightAiUnavailable');
    return t('widgets.preflightUnavailable');
}
function setReadinessState(container, tone, text) {
    container.className = `widget-chat-readiness is-${tone}`;
    container.textContent = text;
}
function setFooterStatus(container, text, tone = 'muted') {
    container.className = `widget-chat-footer-status is-${tone}`;
    container.textContent = text;
}
function renderPreviewState(container, phase, detail = '') {
...
function renderPreviewHtml(container, html, title, phaseLabel, description = '', isPro = false) {
...
function getPreviewHeading(phase) {
    switch (phase) {
