...
function sanitizeData(data) {
    return {
        ...
        newsByCategory: data.newsByCategory
            ? Object.fromEntries(Object.entries(data.newsByCategory).map(([k, items]) => [k, items.map(sanitizeNewsItem)]))
            : undefined,
    };
}
export function exportToJSON(data, filename = 'ivee-export') {
    const jsonStr = JSON.stringify(sanitizeData(data), null, 2);
    downloadFile(jsonStr, `${filename}.json`, 'application/json');
}
export function exportToCSV(data, filename = 'ivee-export') {
