...
function coordsToRegion(lat, lon) {
    if (lat > 15 && lon > 60 && lon < 150)
        return 'asia';
    if (lat > 10 && lat < 45 && lon > 25 && lon < 65)
        return 'mena';
    if (lat > -40 && lat < 40 && lon > -25 && lon < 55)
        return 'africa';
...
function getGeolocationPosition(timeout) {
...
let _countryPromise;
async function resolveCountryCodeInternal() {
    if (!isDesktopRuntime()) {
        try {
...
export function resolveUserCountryCode() {
    if (!_countryPromise)
...
export function resolvePreciseUserCoordinates(timeout = 5000) {
