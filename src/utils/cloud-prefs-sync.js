...
let _cachedToken = null; // synchronous token cache for flush()
// ── Guards ────────────────────────────────────────────────────────────────────
function isEnabled() {
...
// ── Toast ─────────────────────────────────────────────────────────────────────
function showUndoToast(prevBlobJson) {
  ...
  `;
    ...
    toast.addEventListener('click', (e) => {
        ...
        if (action === 'undo') {
            const prev = JSON.parse(prevBlobJson);
            _suppressPatch = true;
            try {
                for (const [k, v] of Object.entries(prev)) {
                    if (CLOUD_SYNC_KEYS.includes(k))
...
async function fetchCloudPrefs(token, variant) {
    ...
    if (res.status === 401)
        return null;
    if (!res.ok)
        throw new Error(`fetch prefs: ${res.status}`);
    return (await res.json());
}
async function postCloudPrefs(token, variant, data, expectedSyncVersion) {
    ...
    if (!res.ok)
        throw new Error(`post prefs: ${res.status}`);
    return (await res.json());
}
// ── Core logic ────────────────────────────────────────────────────────────────
export async function onSignIn(userId, variant) {
    if (!isEnabled())
        return;
...
async function uploadNow(variant) {
    const token = await getClerkToken();
    if (!token)
        return;
    _cachedToken = token;
...
function schedulePrefUpload(variant) {
    setState('pending');
...
export function onPrefChange(variant) {
    if (!isEnabled())
        return;
    _currentVariant = variant;
...
export function install(variant) {
    ...
    Storage.prototype.setItem = function setItem(key, value) {
        ...
        if (this === localStorage && !_suppressPatch && CLOUD_SYNC_KEYS.includes(key)) {
