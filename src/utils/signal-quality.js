...
export function normalizeThreatLevel(level) {
...
export function computeISQ(input, focalPointFn, ciiScoreFn, isFocalDataAvailableFn, profile = 'default') {
    const [wConf, wIntensity, wGap, wTime] = WEIGHTS[profile];
    const confidence = Math.min(1, ((input.sourceCount >= 3 ? 1.0 : input.sourceCount === 2 ? 0.7 : 0.4) +
        (input.isAlert ? 0.2 : 0) +
    ...
    const tier = composite >= 0.75 ? 'strong' :
