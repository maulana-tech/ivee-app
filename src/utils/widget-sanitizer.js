...
function stripLeadingPanelHeader(html) {
    return html.replace(/^\s*<div[^>]*\bclass="panel-header"[^>]*>[\s\S]*?<\/div>\s*/i, '');
}
export function wrapWidgetHtml(html, extraClass = '') {
...
function buildWidgetDoc(bodyContent) {
    return `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
function mountProWidget(iframe) {
    ...
    iframe.addEventListener('load', () => {
        ...
        if (storedHtml)
            iframe.contentWindow?.postMessage({ type: 'wm-html', html: storedHtml }, '*');
    });
}
...
export function wrapProWidgetHtml(bodyContent) {
