...
const GRID_LINES = 4;
const VESSEL_KEYS = ['container', 'dryBulk', 'generalCargo', 'roro', 'tanker'];
const CAP_KEYS = ['capContainer', 'capDryBulk', 'capGeneralCargo', 'capRoro', 'capTanker'];
...
export class TransitChart {
    constructor() {
        ...
        Object.defineProperty(this, "themeHandler", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "allData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        Object.defineProperty(this, "draw", {
            ...
            value: () => {
                ...
                const stacks = data.map(d => (this.tab === 'calls' ? VESSEL_KEYS : CAP_KEYS).map(k => d[k]));
                ...
                const xBar = (i) => PAD.left + (i / data.length) * plotW;
                const yPos = (v) => PAD.top + plotH - (v / yScale) * plotH;
                // Grid
        ...
        Object.defineProperty(this, "onMouseMove", {
            ...
            value: (e) => {
                ...
                const d = data[idx];
                const stack = (this.tab === 'calls' ? VESSEL_KEYS : CAP_KEYS).map(k => d[k]);
                const total = stack.reduce((a, b) => a + b, 0);
                const maValues = compute7dMA(data.map(r => (this.tab === 'calls' ? VESSEL_KEYS : CAP_KEYS).reduce((s, k) => s + r[k], 0)));
                const ma = maValues[idx] ?? 0;
                const fmt = (v) => this.tab === 'dwt' ? fmtDWT(v) : String(v);
                tooltip.innerHTML =
                    `<div style="font-weight:600;margin-bottom:3px">${d.date}</div>` +
                        VESSEL_LABELS.map((label, i) => `<div><span style="color:${VESSEL_COLORS[i]}">■</span> ${label}: ${fmt(stack[i])}</div>`).join('') +
    ...
    buildControls() {
        ...
        const btnStyle = (active) => `font-size:10px;padding:2px 7px;border-radius:3px;cursor:pointer;border:1px solid ${borderSubtle};` +
