...
export function exportToJSON(data: ExportData, filename = 'ave-crypto-export'): void {
  const jsonStr = JSON.stringify(sanitizeData(data), null, 2);
  downloadFile(jsonStr, `${filename}.json`, 'application/json');
}

export function exportToCSV(data: ExportData, filename = 'ave-crypto-export'): void {
  const clean = sanitizeData(data);
  const lines: string[] = [];

  lines.push(`# AVE Crypto Export — ${new Date(clean.timestamp).toISOString()}`);
...
export class ExportPanel {
  ...
  private export(format: ExportFormat): void {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `ave-crypto-${timestamp}`;
