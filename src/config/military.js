...
export const US_MILITARY_CALLSIGNS = [
    // USAF
...
export const NATO_ALLIED_CALLSIGNS = [
    // Royal Air Force (UK)
...
export const ADVERSARY_CALLSIGNS = [
...
export const ALL_MILITARY_CALLSIGNS = [
...
export const MILITARY_VESSEL_PATTERNS = [
...
export const MILITARY_QUERY_REGIONS = [
...
if (import.meta.env.DEV) {
    for (const h of MILITARY_HOTSPOTS) {
        ...
        const covered = MILITARY_QUERY_REGIONS.some(r => r.lamin <= hbox.lamin && r.lamax >= hbox.lamax && r.lomin <= hbox.lomin && r.lomax >= hbox.lomax);
        if (!covered)
            console.error(`[Military] HOTSPOT ${h.name} bbox not covered by any QUERY_REGION`);
    }
}
export const USNI_REGION_COORDINATES = {
    // Seas & Oceans
...
export function normalizeUSNIRegion(regionText) {
    return regionText
        .replace(/^(In the|In|The)\s+/i, '')
        .replace(/\s+/g, ' ')
        .trim();
}
export function getUSNIRegionCoords(regionText) {
...
export function getUSNIRegionApproxCoords(regionText) {
 ...
 */
export function identifyByCallsign(callsign, originCountry) {
 ...
 */
export function identifyByAircraftType(typeCode) {
 ...
 */
export function isKnownMilitaryHex(hexCode) {
 ...
 */
export function getNearbyHotspot(lat, lon) {
