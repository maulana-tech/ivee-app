...
export const SOURCE_PROPAGANDA_RISK = {
...
export function isStateAffiliatedSource(sourceName) {
    const profile = SOURCE_PROPAGANDA_RISK[sourceName];
    return !!profile?.stateAffiliated;
}
let _sourcePanelMap = null;
export function getSourcePanelId(sourceName) {
    if (!_sourcePanelMap) {
        _sourcePanelMap = new Map();
        for (const [category, feeds] of Object.entries(FEEDS)) {
            for (const feed of feeds)
                _sourcePanelMap.set(feed.name, category);
        }
        for (const feed of INTEL_SOURCES)
            _sourcePanelMap.set(feed.name, 'intel');
    }
    ...
    : SITE_VARIANT === 'finance'
        ...
        : SITE_VARIANT === 'happy'
            ...
            : SITE_VARIANT === 'commodity'
                ? COMMODITY_FEEDS
                : FULL_FEEDS;
export const SOURCE_REGION_MAP = {
...
export function getLocaleBoostedSources(locale) {
    ...
    for (const f of allFeeds) {
        if (f.lang === lang)
            boosted.add(f.name);
        if (typeof f.url === 'object' && lang in f.url)
            boosted.add(f.name);
    }
    return boosted;
}
export function computeDefaultDisabledSources(locale) {
    const enabled = getAllDefaultEnabledSources();
    if (locale) {
        for (const name of getLocaleBoostedSources(locale))
            enabled.add(name);
    }
    const all = new Set();
...
export function getTotalFeedCount() {
    const all = new Set();
    for (const feeds of Object.values(FULL_FEEDS))
        for (const f of feeds)
            all.add(f.name);
...
if (import.meta.env.DEV) {
    const allFeedNames = new Set();
    for (const feeds of Object.values(FULL_FEEDS))
        for (const f of feeds)
            allFeedNames.add(f.name);
    for (const f of INTEL_SOURCES)
        allFeedNames.add(f.name);
