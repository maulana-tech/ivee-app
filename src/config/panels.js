...
const FULL_PANELS = {
    ...
    forecast: { name: 'AI Forecasts', enabled: true, priority: 1, ...(_desktop && { premium: 'locked' }) }, // trial: unlocked on web, locked on desktop
    cii: { name: 'Country Instability', enabled: true, priority: 1, ...(_desktop && { premium: 'enhanced' }) },
    'strategic-risk': { name: 'Strategic Risk Overview', enabled: true, priority: 1, ...(_desktop && { premium: 'enhanced' }) },
    intel: { name: 'Intel Feed', enabled: true, priority: 1 },
    'gdelt-intel': { name: 'Live Intelligence', enabled: true, priority: 1, ...(_desktop && { premium: 'enhanced' }) },
    ...
    'chat-analyst': { name: 'WM Analyst', enabled: true, priority: 1, premium: 'locked' },
    economic: { name: 'Macro Stress', enabled: true, priority: 1 },
    'trade-policy': { name: 'Trade Policy', enabled: true, priority: 1 },
    'supply-chain': { name: 'Supply Chain', enabled: true, priority: 1, ...(_desktop && { premium: 'enhanced' }) },
    ...
    'oref-sirens': { name: 'Israel Sirens', enabled: true, priority: 2, ...(_desktop && { premium: 'locked' }) },
    'telegram-intel': { name: 'Telegram Intel', enabled: true, priority: 2, ...(_desktop && { premium: 'locked' }) },
    ...
    'market-implications': { name: 'AI Market Implications', enabled: true, priority: 1, premium: 'locked' },
...
const CRYPTO_PANELS = {
    ...
    'whale-alerts': { name: 'Whale Alerts', enabled: true, priority: 1 },
    'portfolio': { name: 'Portfolio', enabled: true, priority: 1 },
    signals: { name: 'Trading Signals', enabled: true, priority: 1 },
    'risk-scanner': { name: 'Risk Scanner', enabled: true, priority: 1 },
    trending: { name: 'Trending Tokens', enabled: true, priority: 1 },
    'crypto-news': { name: 'Crypto News', enabled: true, priority: 2 },
...
const CRYPTO_MAP_LAYERS = {
    ...
    displacement: false,
    climate: false,
    // Tech layers (show datacenters for crypto infrastructure)
    startupHubs: false,
    cloudRegions: true,
    accelerators: false,
    techHQs: false,
...
const CRYPTO_MOBILE_MAP_LAYERS = {
    gpsJamming: false,
    satellites: false,
    conflicts: false,
    bases: false,
...
const COMMODITY_MOBILE_MAP_LAYERS = {
    gpsJamming: false,
    satellites: false,
    conflicts: false,
    bases: false,
 ...
 */
export const VARIANT_PANEL_OVERRIDES = {
    finance: {
    ...
    crypto: {
        map: { name: 'Crypto Map' },
        'live-news': { name: 'Crypto Headlines' },
        insights: { name: 'AI Trading Insights' },
    },
    happy: {
        map: { name: 'World Map' },
    },
 ...
 */
export function getEffectivePanelConfig(key, variant) {
    const base = ALL_PANELS[key];
    if (!base)
        return { name: key, enabled: false, priority: 2 };
 ...
 */
export function isPanelEntitled(key, config, isPro = false) {
    if (!config.premium)
        return true;
...
// ============================================
export const DEFAULT_PANELS = Object.fromEntries((VARIANT_DEFAULTS[SITE_VARIANT] ?? VARIANT_DEFAULTS['full'] ?? []).map(key => [key, getEffectivePanelConfig(key, SITE_VARIANT)]));
...
export const PANEL_CATEGORY_MAP = {
    ...
    // Crypto variant
    whaleIntelligence: {
        labelKey: 'header.panelCatWhaleIntel',
        panelKeys: ['whale-alerts', 'signals', 'trending', 'risk-scanner'],
    },
    cryptoPortfolio: {
        labelKey: 'header.panelCatCryptoPortfolio',
        panelKeys: ['portfolio', 'crypto', 'defi-tokens', 'ai-tokens', 'stablecoins'],
    },
    cryptoMarkets: {
