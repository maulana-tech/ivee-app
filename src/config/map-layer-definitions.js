...
const def = (key, icon, i18nSuffix, fallbackLabel, renderers = ['flat', 'globe'], premium) => ({ key, icon, i18nSuffix, fallbackLabel, renderers, ...(premium && { premium }) });
...
export function getLayersForVariant(variant, renderer) {
...
export function getAllowedLayerKeys(variant) {
    return new Set(VARIANT_LAYER_ORDER[variant] ?? VARIANT_LAYER_ORDER.full);
}
export function sanitizeLayersForVariant(layers, variant) {
...
export function resolveLayerLabel(def, tFn) {
    if (tFn) {
...
export function bindLayerSearch(container) {
    ...
    searchInput.addEventListener('input', () => {
        ...
        if (q) {
            for (const [alias, keys] of Object.entries(LAYER_SYNONYMS)) {
                if (alias.includes(q))
                    keys.forEach(k => synonymHits.add(k));
            }
        }
        container.querySelectorAll('.layer-toggle').forEach(label => {
            const el = label;
            if (el.hasAttribute('data-layer-hidden'))
                return;
            if (!q) {
                el.style.display = '';
                return;
