...
function appendDebugLog(entry) {
    try {
        ...
        log.push(entry);
        if (log.length > SW_DEBUG_LOG_MAX)
            log.splice(0, log.length - SW_DEBUG_LOG_MAX);
 ...
 */
export function installSwUpdateHandler(options = {}) {
    const swContainer = options.swContainer ?? navigator.serviceWorker;
    const doc = options.document ?? document;
    const reload = options.reload ?? (() => window.location.reload());
    const raf = options.raf ?? ((cb) => requestAnimationFrame(() => requestAnimationFrame(cb)));
    const setTimer = options.setTimer ?? ((cb, ms) => setTimeout(cb, ms));
    const clearTimer = options.clearTimer ?? ((id) => { if (id !== null)
        clearTimeout(id); });
    ...
    const VISIBLE_DWELL_MS = 5000;
    let currentOnHidden = null;
    let currentDwellCancel = null;
    const showToast = () => {
        if (currentOnHidden) {
    ...
    `;
        ...
        const startDwellTimer = () => {
            if (dwellTimerId !== null || dismissed || autoReloadAllowed)
                return;
