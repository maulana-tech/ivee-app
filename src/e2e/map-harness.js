...
import { SITE_VARIANT, INTEL_HOTSPOTS, CONFLICT_ZONES, MILITARY_BASES, UNDERSEA_CABLES, NUCLEAR_FACILITIES, GAMMA_IRRADIATORS, PIPELINES, STRATEGIC_WATERWAYS, ECONOMIC_CENTERS, AI_DATA_CENTERS, STARTUP_HUBS, ACCELERATORS, TECH_HQS, CLOUD_REGIONS, PORTS, SPACEPORTS, APT_GROUPS, CRITICAL_MINERALS, STOCK_EXCHANGES, FINANCIAL_CENTERS, CENTRAL_BANKS, COMMODITY_HUBS, } from '../config';
...
const internals = map;
const buildLayerState = (enabledLayers) => {
    const next = { ...allLayersDisabled };
    for (const key of enabledLayers) {
        next[key] = true;
    }
    return next;
};
const setLayersForSnapshot = (enabledLayers) => {
...
const getLayerDataCount = (layerId) => {
    return getDeckLayerSnapshot().find((layer) => layer.id === layerId)?.dataCount ?? 0;
};
const getLayerFirstScreenTransform = (layerId) => {
    const maplibreMap = internals.maplibreMap;
    if (!maplibreMap)
        return null;
    const layers = internals.buildLayers?.() ?? [];
    ...
    if (!Number.isFinite(lon) || !Number.isFinite(lat))
        return null;
    const point = maplibreMap.project([lon, lat]);
    return `translate(${point.x.toFixed(2)}px, ${point.y.toFixed(2)}px)`;
};
const getFirstProtestTitle = () => {
    const layers = internals.buildLayers?.() ?? [];
...
const toCamera = (lon, lat, zoom) => ({
    lon,
    lat,
    zoom,
});
const firstLatLon = (items, fallback) => {
    const first = items[0];
    if (!first)
        return fallback;
    return [first.lon, first.lat];
};
const firstPathPoint = (items, fallback) => {
    const firstPoint = items[0]?.points?.[0];
    if (!firstPoint || firstPoint.length < 2)
        return fallback;
    return [firstPoint[0], firstPoint[1]];
};
const firstConflictPoint = (fallback) => {
...
const filterScenariosForVariant = (variant) => {
    return VISUAL_SCENARIOS.filter((scenario) => scenario.variant === 'both' || scenario.variant === variant);
};
const currentHarnessVariant = SITE_VARIANT === 'tech'
    ? 'tech'
    : SITE_VARIANT === 'finance'
        ? 'finance'
        : 'full';
const buildProtests = (scenario) => {
...
const buildPulseProtests = (scenario) => {
    if (scenario === 'none')
        return [];
    const now = new Date();
...
const buildHotspotActivityNews = (scenario) => {
    if (scenario === 'none')
        return [];
    return [
        {
...
const makeNewsLocationsNonRecent = () => {
    const now = Date.now();
    if (internals.newsLocationFirstSeen) {
        for (const key of internals.newsLocationFirstSeen.keys()) {
            internals.newsLocationFirstSeen.set(key, now - 120000);
        }
    }
    internals.stopPulseAnimation?.();
};
const setNewsPulseScenario = (scenario) => {
    if (scenario === 'none') {
...
const ensureDeterministicStyles = () => {
    if (document.getElementById(DETERMINISTIC_STYLE_ID))
        return;
...
const enableDeterministicVisualMode = () => {
...
const prepareVisualScenario = (scenarioId) => {
...
const isVisualScenarioReady = (scenarioId) => {
    const scenario = visualScenarioMap.get(scenarioId);
    if (!scenario)
        return false;
    const layersById = new Map(getDeckLayerSnapshot().map((layer) => [layer.id, layer.dataCount]));
...
const getCyberTooltipHtml = (indicator) => {
...
window.__mapHarness = {
    ...
    seedAllDynamicData,
    setProtestsScenario: (scenario) => {
        map.setProtests(buildProtests(scenario));
    },
    setPulseProtestsScenario: (scenario) => {
        map.setProtests(buildPulseProtests(scenario));
    },
    setNewsPulseScenario,
    setHotspotActivityScenario: (scenario) => {
        map.updateHotspotActivity(buildHotspotActivityNews(scenario));
    },
    forcePulseStartupElapsed: () => {
        internals.startupTime = Date.now() - 61000;
    },
    resetPulseStartupTime: () => {
        internals.startupTime = Date.now();
    },
    isPulseAnimationRunning: () => {
