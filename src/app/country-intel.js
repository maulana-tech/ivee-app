...
import { getCountryAtCoordinates, getCountryCentroid, hasCountryGeometry, isCoordinateInCountry, ME_STRIKE_BOUNDS, iso3ToIso2Code, nameToCountryCode, } from '@/services/country-geometry';
import { calculateCII, getCountryData, TIER1_COUNTRIES, hasIntelligenceSignalsLoaded } from '@/services/country-instability';
...
export class CountryIntelManager {
    constructor(ctx) {
        ...
        Object.defineProperty(this, "briefRequestToken", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "frameworkUnsubscribe", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "_fwDebounce", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
    ...
    async openCountryBrief(lat, lon) {
        if (!this.ctx.countryBriefPage)
            return;
    ...
    async openCountryBriefByCode(code, country, opts) {
        if (!this.ctx.countryBriefPage)
            return;
        this.ctx.map?.setRenderPaused(true);
        ...
        const marketClient = new MarketServiceClient(getRpcBaseUrl(), { fetch: (...args) => globalThis.fetch(...args) });
        const stockPromise = marketClient.getCountryStockIndex({ countryCode: code })
            ...
            .catch(() => ({ available: false, code: '', symbol: '', indexName: '', price: '0', weekChangePercent: '0', currency: '' }));
        ...
        try {
            ...
            const briefHeadlines = context.headlines || [];
            ...
            try {
                ...
                if (isHeadlineMemoryEnabled() && mlWorker.isAvailable && mlWorker.isModelLoaded('embeddings') && briefHeadlines.length > 0) {
                    try {
                        ...
                        if (results.length > 0) {
                            const historical = results.map(r => `- ${r.text} (${new Date(r.pubDate).toISOString().slice(0, 10)})`).join('\n').slice(0, 350);
            ...
            if (briefText) {
                ...
                if (fallbackBrief) {
                    ...
                    const lines = [];
                    if (score)
                        lines.push(t('countryBrief.fallback.instabilityIndex', { score: String(score.score), level: t(`countryBrief.levels.${score.level}`), trend: t(`countryBrief.trends.${score.trend}`) }));
                    if (signals.protests > 0)
                        lines.push(t('countryBrief.fallback.protestsDetected', { count: String(signals.protests) }));
                    if (signals.militaryFlights > 0)
                        lines.push(t('countryBrief.fallback.aircraftTracked', { count: String(signals.militaryFlights) }));
                    if (signals.militaryVessels > 0)
                        lines.push(t('countryBrief.fallback.vesselsTracked', { count: String(signals.militaryVessels) }));
                    if (signals.activeStrikes > 0)
                        lines.push(t('countryBrief.fallback.activeStrikes', { count: String(signals.activeStrikes) }));
                    ...
                    if (signals.outages > 0)
                        lines.push(t('countryBrief.fallback.internetOutages', { count: String(signals.outages) }));
                    ...
                    if (signals.earthquakes > 0)
                        lines.push(t('countryBrief.fallback.recentEarthquakes', { count: String(signals.earthquakes) }));
    ...
    refreshOpenBrief() {
        ...
        if (!page?.isVisible())
            return;
        const code = page.getCode();
        if (!code || code === '__loading__' || code === '__error__')
            return;
    ...
    async fetchCountryIntelBrief(code, contextSnapshot, framework = '') {
        const lang = getCurrentLanguage();
    ...
    buildBriefContextSnapshot(country, code, score, signals, context) {
        const lines = [];
        ...
        if (stockIndex)
            lines.push(`Stock index: ${stockIndex}`);
        const convergenceScore = typeof context.convergenceScore === 'number' ? context.convergenceScore : null;
        const signalTypes = Array.isArray(context.signalTypes) ? context.signalTypes : [];
        ...
        const regionalConvergence = Array.isArray(context.regionalConvergence) ? context.regionalConvergence : [];
    ...
    buildInfrastructureContext(code) {
        try {
            ...
            if (pipes.length)
                parts.push(`Pipelines: ${pipes.join(', ')}`);
            ...
            const chokepoints = incomingEdges
                ...
                .map(e => {
                const node = graph.nodes.get(e.from);
                const reason = e.metadata?.relationship;
                return node ? `${node.name}${reason ? ` (${reason})` : ''}` : '';
            }).filter(Boolean)
                .slice(0, 2);
            if (chokepoints.length)
                parts.push(`Waterways: ${chokepoints.join(', ')}`);
    ...
    mountCountryTimeline(code, country) {
        this.ctx.countryTimeline?.destroy();
        this.ctx.countryTimeline = null;
        ...
        const inCountry = (lat, lon) => hasGeoShape && this.isInCountry(lat, lon, code);
    ...
    getCountrySignals(code, country) {
        const countryLower = country.toLowerCase();
        const hasGeoShape = hasCountryGeometry(code) || !!CountryIntelManager.COUNTRY_BOUNDS[code];
        ...
        if (countryCluster) {
            for (const s of countryCluster.signals) {
                if (s.type === 'ais_disruption')
                    signalTypeCounts.aisDisruptions++;
                else if (s.type === 'satellite_fire')
                    signalTypeCounts.satelliteFires++;
                else if (s.type === 'radiation_anomaly')
                    signalTypeCounts.radiationAnomalies++;
        ...
        let protests = 0;
        if (this.ctx.intelligenceCache.protests?.events) {
            protests = this.ctx.intelligenceCache.protests.events.filter((e) => e.country?.toLowerCase() === countryLower || (hasGeoShape && this.isInCountry(e.lat, e.lon, code))).length;
        }
        let militaryFlights = 0;
        let militaryVessels = 0;
        if (this.ctx.intelligenceCache.military) {
            militaryFlights = this.ctx.intelligenceCache.military.flights.filter((f) => hasGeoShape ? this.isInCountry(f.lat, f.lon, code) : f.operatorCountry?.toUpperCase() === code).length;
            militaryVessels = this.ctx.intelligenceCache.military.vessels.filter((v) => hasGeoShape ? this.isInCountry(v.lat, v.lon, code) : v.operatorCountry?.toUpperCase() === code).length;
        }
        let outages = 0;
        if (this.ctx.intelligenceCache.outages) {
            outages = this.ctx.intelligenceCache.outages.filter((o) => o.country?.toLowerCase() === countryLower || (hasGeoShape && this.isInCountry(o.lat, o.lon, code))).length;
        }
        let earthquakes = 0;
        if (this.ctx.intelligenceCache.earthquakes) {
        ...
        if (this.ctx.intelligenceCache.flightDelays) {
            aviationDisruptions = this.ctx.intelligenceCache.flightDelays.filter(d => (d.severity === 'major' || d.severity === 'severe' || d.delayType === 'closure') &&
                (hasGeoShape ? this.isInCountry(d.lat, d.lon, code) : d.country?.toLowerCase() === countryLower)).length;
        }
        const ciiData = getCountryData(code);
        ...
        let travelAdvisoryMaxLevel = null;
        const advisoryLevelRank = { 'do-not-travel': 4, 'reconsider': 3, 'caution': 2, 'normal': 1, 'info': 0 };
        if (this.ctx.intelligenceCache.advisories) {
            const countryAdvisories = this.ctx.intelligenceCache.advisories.filter(a => a.country === code);
        ...
        if (this.ctx.intelligenceCache.thermalEscalation) {
            thermalEscalations = this.ctx.intelligenceCache.thermalEscalation.clusters.filter((c) => c.countryCode.toUpperCase() === code && c.status !== 'normal').length;
        }
        const sanctionsCountry = this.ctx.intelligenceCache.sanctions?.countries.find((c) => c.countryCode.toUpperCase() === code);
    ...
    buildMilitarySummary(code, country) {
        const hasGeoShape = hasCountryGeometry(code) || !!CountryIntelManager.COUNTRY_BOUNDS[code];
        const flights = this.ctx.intelligenceCache.military?.flights ?? [];
        const vessels = this.ctx.intelligenceCache.military?.vessels ?? [];
        const flightsInCountry = flights.filter((flight) => hasGeoShape ? this.isInCountry(flight.lat, flight.lon, code) : this.sameCountry(code, country, flight.operatorCountry));
        ...
        const vesselsInCountry = vessels.filter((vessel) => hasGeoShape ? this.isInCountry(vessel.lat, vessel.lon, code) : this.sameCountry(code, country, vessel.operatorCountry));
    ...
    buildEconomicIndicators(code, score, stock) {
        const indicators = [];
        if (stock?.available) {
        ...
        if (countryData?.displacementOutflow && countryData.displacementOutflow > 0) {
            const displaced = countryData.displacementOutflow >= 1000000
                ? `${(countryData.displacementOutflow / 1000000).toFixed(1)}M`
    ...
    mapSignalType(type) {
        ...
        if (type === 'internet_outage')
            return 'OUTAGE';
        if (type === 'satellite_fire')
            return 'DISASTER';
        if (type === 'radiation_anomaly')
            return 'DISASTER';
        if (type === 'ais_disruption')
            return 'OUTAGE';
        if (type === 'active_strike')
            return 'MILITARY';
        if (type === 'temporal_anomaly')
            return 'CYBER';
        return 'OTHER';
    }
    normalizeSignalSeverity(type, severity) {
        if (type === 'active_strike' && severity === 'high')
            return 'critical';
        if (type === 'radiation_anomaly' && severity === 'high')
            return 'critical';
        if (severity === 'high')
            return 'high';
        if (severity === 'medium')
    ...
    getCountryStrikes(code, hasGeoShape) {
    ...
    static firstMentionPosition(text, terms) {
    ...
    static getOtherCountryTerms(code) {
    ...
    static resolveCountryName(code) {
        if (TIER1_COUNTRIES[code])
            return TIER1_COUNTRIES[code];
        try {
    ...
    static getCountrySearchTerms(country, code) {
...
Object.defineProperty(CountryIntelManager, "COUNTRY_BOUNDS", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
...
Object.defineProperty(CountryIntelManager, "COUNTRY_ALIASES", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
...
Object.defineProperty(CountryIntelManager, "otherCountryTermsCache", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: new Map()
