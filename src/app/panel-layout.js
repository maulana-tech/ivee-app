...
import { MapContainer, NewsPanel, MarketPanel, StockAnalysisPanel, StockBacktestPanel, EconomicPanel, YieldCurvePanel, EarningsCalendarPanel, EconomicCalendarPanel, CotPositioningPanel, SupplyChainPanel, TechEventsPanel, } from '@/components';
import { focusInvestmentOnMap } from '@/services/investments-focus';
import { debounce, saveToStorage, loadFromStorage } from '@/utils';
import { escapeHtml } from '@/utils/sanitize';
import { FEEDS, INTEL_SOURCES, STORAGE_KEYS, SITE_VARIANT, ALL_PANELS, VARIANT_DEFAULTS, } from '@/config';
...
export class PanelLayoutManager {
    constructor(ctx, callbacks) {
        ...
        Object.defineProperty(this, "panelDragCleanupHandlers", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "resolvedPanelOrder", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "bottomSetMemory", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Set()
        });
        Object.defineProperty(this, "criticalBannerEl", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "aviationCommandBar", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "applyTimeRangeFilterDebounced", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "proBlockUnsubscribe", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "boundWidgetCreatorHandler", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "unsubscribeEntitlementChange", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "unsubscribePaymentFailureBanner", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "wasUltraWide", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
    ...
    init() {
        ...
        this.boundWidgetCreatorHandler = ((e) => {
            openWidgetChatModal({
                mode: 'create',
                tier: 'pro',
    ...
    updatePanelGating(state) {
        for (const [key, panel] of Object.entries(this.ctx.panels)) {
            ...
            if (reason === PanelGateReason.NONE) {
                ...
                panel.showGatedCta(reason, onAction);
            }
        }
    }
    /** Return the action callback for a given gate reason. */
    getGateAction(reason) {
        switch (reason) {
    ...
    async fetchGitHubStars() {
        try {
            const response = await fetch('https://api.github.com/repos/koala73/ivee');
            if (!response.ok)
                return;
    ...
    renderLayout() {
      ...
      <div class="header">
        <div class="header-left">
          ...
          <div class="variant-switcher">${(() => {
            ...
            const vHref = (v, prod) => local || SITE_VARIANT === v ? '#' : prod;
            const vTarget = (v) => !local && SITE_VARIANT !== v && inIframe ? 'target="_blank" rel="noopener"' : '';
            return `
          ...
          <a href="https://github.com/koala73/ivee" target="_blank" rel="noopener" class="github-link" title="${t('header.viewOnGitHub')}">
      ...
      <nav class="mobile-menu" id="mobileMenu">
        ...
        ${(() => {
            ...
            return variants.map(v => `<button class="mobile-menu-item mobile-menu-variant ${v.key === SITE_VARIANT ? 'active' : ''}" data-variant="${v.key}">
        ...
        <div class="mobile-menu-footer-links">
          <a href="${this.ctx.isDesktopApp ? 'https://ivee.app/pro' : 'https://www.ivee.app/pro'}" target="_blank" rel="noopener">Pro</a>
          <a href="${this.ctx.isDesktopApp ? 'https://ivee.app/blog/' : 'https://www.ivee.app/blog/'}" target="_blank" rel="noopener">Blog</a>
          <a href="${this.ctx.isDesktopApp ? 'https://ivee.app/docs' : 'https://www.ivee.app/docs'}" target="_blank" rel="noopener">Docs</a>
          <a href="https://status.ivee.app/" target="_blank" rel="noopener">Status</a>
        </div>
      ...
      <div class="region-bottom-sheet" id="regionBottomSheet">
        ...
        ].map(r => `<button class="region-sheet-option ${r.value === 'global' ? 'active' : ''}" data-region="${r.value}">
          <span>${r.label}</span>
          ...
          <div class="panel-header">
            ...
            <div class="map-header-actions">
              <div class="map-dimension-toggle" id="mapDimensionToggle">
                <button class="map-dim-btn${loadFromStorage(STORAGE_KEYS.mapMode, 'flat') === 'globe' ? '' : ' active'}" data-mode="flat" title="2D Map">2D</button>
                <button class="map-dim-btn${loadFromStorage(STORAGE_KEYS.mapMode, 'flat') === 'globe' ? ' active' : ''}" data-mode="globe" title="3D Globe">3D</button>
              </div>
      ...
      <footer class="site-footer">
        ...
        <nav>
          ...
          <a href="${this.ctx.isDesktopApp ? 'https://ivee.app/docs' : 'https://www.ivee.app/docs'}" target="_blank" rel="noopener">Docs</a>
          <a href="https://status.ivee.app/" target="_blank" rel="noopener">Status</a>
          <a href="https://github.com/koala73/ivee" target="_blank" rel="noopener">GitHub</a>
          <a href="https://discord.gg/re63kWKxaz" target="_blank" rel="noopener">Discord</a>
          <a href="https://x.com/iveeai" target="_blank" rel="noopener">X</a>
    ...
    setupMobileMapToggle() {
        ...
        const updateBtn = (btn, isCollapsed) => {
    ...
    renderCriticalBanner(postures) {
        ...
        const critical = postures.filter((p) => p.postureLevel === 'critical' || (p.postureLevel === 'elevated' && p.strikeCapable));
     ...
     */
    mountLiveNewsIfReady() {
        if (this.ctx.panels['live-news'])
            return;
        if (getDefaultLiveChannels().length === 0 && loadChannelsFromStorage().length === 0)
            return;
    ...
    shouldCreatePanel(key) {
        return Object.prototype.hasOwnProperty.call(this.ctx.panelSettings, key);
    }
    createNewsPanel(key, labelKey) {
        if (!this.shouldCreatePanel(key))
            return null;
    ...
    static computeEventRisk(cluster) {
        if (!cluster.threat)
            return null;
        const levelScore = { critical: 95, high: 75, medium: 50, low: 25, info: 10 };
    ...
    createPanels() {
        ...
        const preferGlobe = loadFromStorage(STORAGE_KEYS.mapMode, 'flat') === 'globe';
        ...
        this.lazyPanel('displacement', () => import('@/components/DisplacementPanel').then(m => {
            const p = new m.DisplacementPanel();
            p.setCountryClickHandler((lat, lon) => { this.ctx.map?.setCenter(lat, lon, 4); });
            return p;
        }));
        this.lazyPanel('climate', () => import('@/components/ClimateAnomalyPanel').then(m => {
            const p = new m.ClimateAnomalyPanel();
            p.setZoneClickHandler((lat, lon) => { this.ctx.map?.setCenter(lat, lon, 4); });
            return p;
        }));
        this.lazyPanel('population-exposure', () => import('@/components/PopulationExposurePanel').then(m => new m.PopulationExposurePanel()));
        this.lazyPanel('security-advisories', () => import('@/components/SecurityAdvisoriesPanel').then(m => {
            const p = new m.SecurityAdvisoriesPanel();
            p.setRefreshHandler(() => { void this.callbacks.loadSecurityAdvisories?.(); });
            return p;
        }));
        this.lazyPanel('radiation-watch', () => import('@/components/RadiationWatchPanel').then(m => {
            const p = new m.RadiationWatchPanel();
            p.setLocationClickHandler((lat, lon) => { this.ctx.map?.setCenter(lat, lon, 4); });
            return p;
        }));
        this.lazyPanel('thermal-escalation', () => import('@/components/ThermalEscalationPanel').then(m => {
            const p = new m.ThermalEscalationPanel();
            p.setLocationClickHandler((lat, lon) => { this.ctx.map?.setCenter(lat, lon, 4); });
            return p;
        }));
        const _lockPanels = this.ctx.isDesktopApp && !hasPremiumAccess();
        this.lazyPanel('daily-market-brief', () => import('@/components/DailyMarketBriefPanel').then(m => new m.DailyMarketBriefPanel()));
        this.lazyPanel('market-implications', () => import('@/components/MarketImplicationsPanel').then(m => new m.MarketImplicationsPanel()));
        ...
        this.lazyPanel('chat-analyst', () => import('@/components/ChatAnalystPanel').then(m => new m.ChatAnalystPanel()));
        this.lazyPanel('forecast', () => import('@/components/ForecastPanel').then(m => new m.ForecastPanel()), undefined, _lockPanels ? ['AI-powered geopolitical forecasts', 'Cross-domain cascade predictions', 'Prediction market calibration'] : undefined);
        this.lazyPanel('oref-sirens', () => import('@/components/OrefSirensPanel').then(m => new m.OrefSirensPanel()), undefined, _lockPanels ? [t('premium.features.orefSirens1'), t('premium.features.orefSirens2')] : undefined);
        this.lazyPanel('telegram-intel', () => import('@/components/TelegramIntelPanel').then(m => new m.TelegramIntelPanel()), undefined, _lockPanels ? [t('premium.features.telegramIntel1'), t('premium.features.telegramIntel2')] : undefined);
        ...
        this.lazyPanel('tech-readiness', () => import('@/components/TechReadinessPanel').then(m => {
            const p = new m.TechReadinessPanel();
            void p.refresh();
            return p;
        }));
        this.lazyPanel('national-debt', () => import('@/components/NationalDebtPanel').then(m => {
            const p = new m.NationalDebtPanel();
            void p.refresh();
            return p;
        }));
        this.lazyPanel('cross-source-signals', () => import('@/components/CrossSourceSignalsPanel').then(m => new m.CrossSourceSignalsPanel()));
        this.lazyPanel('geo-hubs', () => import('@/components/GeoHubsPanel').then(m => {
            const p = new m.GeoHubsPanel();
            p.setOnHubClick((hub) => { this.ctx.map?.setCenter(hub.lat, hub.lon, 4); });
            return p;
        }));
        this.lazyPanel('tech-hubs', () => import('@/components/TechHubsPanel').then(m => {
            const p = new m.TechHubsPanel();
            p.setOnHubClick((hub) => { this.ctx.map?.setCenter(hub.lat, hub.lon, 4); });
            return p;
        }));
        this.lazyPanel('ai-regulation', () => import('@/components/RegulationPanel').then(m => new m.RegulationPanel('ai-regulation')));
        ...
        this.lazyPanel('giving', () => import('@/components/GivingPanel').then(m => new m.GivingPanel()));
        // Happy variant panels (lazy-loaded — only relevant for happy variant)
        if (SITE_VARIANT === 'happy') {
            this.lazyPanel('positive-feed', () => import('@/components/PositiveNewsFeedPanel').then(m => {
                const p = new m.PositiveNewsFeedPanel();
                this.ctx.positivePanel = p;
                return p;
            }));
            this.lazyPanel('counters', () => import('@/components/CountersPanel').then(m => {
                const p = new m.CountersPanel();
                p.startTicking();
                this.ctx.countersPanel = p;
                return p;
            }));
            this.lazyPanel('progress', () => import('@/components/ProgressChartsPanel').then(m => {
                const p = new m.ProgressChartsPanel();
                this.ctx.progressPanel = p;
                return p;
            }));
            this.lazyPanel('breakthroughs', () => import('@/components/BreakthroughsTickerPanel').then(m => {
                const p = new m.BreakthroughsTickerPanel();
                this.ctx.breakthroughsPanel = p;
                return p;
            }));
            this.lazyPanel('spotlight', () => import('@/components/HeroSpotlightPanel').then(m => {
                const p = new m.HeroSpotlightPanel();
                p.onLocationRequest = (lat, lon) => {
                    this.ctx.map?.setCenter(lat, lon, 4);
            ...
            this.lazyPanel('digest', () => import('@/components/GoodThingsDigestPanel').then(m => {
                const p = new m.GoodThingsDigestPanel();
                this.ctx.digestPanel = p;
                return p;
            }));
            this.lazyPanel('species', () => import('@/components/SpeciesComebackPanel').then(m => {
                const p = new m.SpeciesComebackPanel();
                this.ctx.speciesPanel = p;
                return p;
            }));
            this.lazyPanel('renewable', () => import('@/components/RenewableEnergyPanel').then(m => {
    ...
    applyInitialUrlState() {
        if (!this.ctx.initialUrlState || !this.ctx.map)
            return;
    ...
    buildUnifiedOrder(sidebarIds, bottomIds) {
        ...
        const addConstraints = (ids) => {
            for (let i = 1; i < ids.length; i++) {
                const prev = ids[i - 1];
                const next = ids[i];
                if (prev === next || !seen.has(prev) || !seen.has(next))
                    continue;
                const nextIds = edges.get(prev);
                if (!nextIds || nextIds.has(next))
                    continue;
                nextIds.add(next);
        ...
        const compareIds = (a, b) => (previousOrder.get(a) ?? Number.MAX_SAFE_INTEGER) - (previousOrder.get(b) ?? Number.MAX_SAFE_INTEGER);
    ...
    getSavedBottomSet() {
        ...
        try {
            ...
            if (legacy) {
                ...
                if (Array.isArray(parsed)) {
                    const bottomIds = parsed.filter((v) => typeof v === 'string');
    ...
    ensureCorrectZones() {
        const effectiveUltraWide = this.getEffectiveUltraWide();
        if (effectiveUltraWide === this.wasUltraWide)
            return;
        this.wasUltraWide = effectiveUltraWide;
        const grid = document.getElementById('panelsGrid');
        const bottomGrid = document.getElementById('mapBottomGrid');
        ...
        if (!effectiveUltraWide) {
            const panelsInBottom = Array.from(bottomGrid.querySelectorAll('.panel'));
            ...
            this.bottomSetMemory.forEach(id => {
                ...
                if (el) {
                    this.insertByOrder(bottomGrid, el, id);
                }
            });
        }
    }
    ...
    lazyPanel(key, loader, setup, lockedFeatures) {
        if (!this.shouldCreatePanel(key))
            return;
    ...
    makeDraggable(el, key) {
        ...
        const onMouseDown = (e) => {
            ...
            if (el.dataset.resizing === 'true')
                return;
            if (target.classList?.contains('panel-resize-handle') ||
        ...
        const findDropPosition = (clientX, clientY) => {
            const grid = document.getElementById('panelsGrid');
            const bottomGrid = document.getElementById('mapBottomGrid');
            if (!grid || !bottomGrid)
                return null;
            ...
            if (!target)
                return null;
            const targetGrid = (target.closest('.panels-grid') || target.closest('.map-bottom-grid'));
            const targetPanel = target.closest('.panel');
            if (!targetGrid && !targetPanel)
                return null;
            const currentTargetGrid = targetGrid || (targetPanel ? targetPanel.parentElement : null);
    ...
    getLocalizedPanelName(panelKey, fallback) {
        if (panelKey === 'runtime-config') {
            return t('modals.runtimeConfig.title');
        }
        const key = panelKey.replace(/-([a-z])/g, (_match, group) => group.toUpperCase());
    ...
    getAllSourceNames() {
        ...
        Object.values(FEEDS).forEach(feeds => {
            if (feeds)
                feeds.forEach(f => sources.add(f.name));
        });
        INTEL_SOURCES.forEach(f => sources.add(f.name));
        return Array.from(sources).sort((a, b) => a.localeCompare(b));
    }
}
Object.defineProperty(PanelLayoutManager, "NEWS_PANEL_TOOLTIPS", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
