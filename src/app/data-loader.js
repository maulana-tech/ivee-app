...
import { FEEDS, INTEL_SOURCES, SECTORS, COMMODITIES, MARKET_SYMBOLS, SITE_VARIANT, LAYER_TO_SOURCE, } from '@/config';
import { INTEL_HOTSPOTS, CONFLICT_ZONES } from '@/config/geo';
import { tokenizeForMatch, matchKeyword } from '@/utils/keyword-match';
import { fetchCategoryFeeds, getFeedFailures, fetchMultipleStocks, fetchCommodityQuotes, fetchSectors, warmCommodityCache, warmSectorCache, fetchCrypto, fetchCryptoSectors, fetchDefiTokens, fetchAiTokens, fetchOtherTokens, fetchPredictions, fetchEarthquakes, fetchWeatherAlerts, fetchFredData, fetchInternetOutages, fetchTrafficAnomalies, fetchDdosAttacks, isOutagesConfigured, fetchAisSignals, getAisStatus, isAisConfigured, fetchCableActivity, fetchCableHealth, fetchProtestEvents, getProtestStatus, fetchFlightDelays, fetchMilitaryFlights, fetchMilitaryVessels, initMilitaryVesselStream, isMilitaryVesselTrackingConfigured, fetchUSNIFleetReport, updateBaseline, calculateDeviation, addToSignalHistory, analysisWorker, fetchPizzIntStatus, fetchGdeltTensions, fetchNaturalEvents, fetchRecentAwards, fetchOilAnalytics, fetchCrudeInventoriesRpc, fetchNatGasStorageRpc, getEuGasStorageData, getOilStocksAnalysisData, fetchLngVulnerability, getEcbFxRatesData, fetchBisData, fetchBlsData, fetchCyberThreats, drainTrendingSignals, fetchTradeRestrictions, fetchTariffTrends, fetchTradeFlows, fetchComtradeFlows, fetchTradeBarriers, fetchCustomsRevenue, fetchShippingRates, fetchChokepointStatus, fetchCriticalMinerals, fetchSanctionsPressure, fetchRadiationWatch, } from '@/services';
import { getMarketWatchlistEntries } from '@/services/market-watchlist';
import { fetchStockAnalysesForTargets, getStockAnalysisTargets } from '@/services/stock-analysis';
import { fetchStockBacktestsForTargets, fetchStoredStockBacktests, getMissingOrStaleStoredStockBacktests, hasFreshStoredStockBacktests, } from '@/services/stock-backtest';
import { fetchStockAnalysisHistory, getMissingOrStaleStockAnalysisSymbols, hasFreshStockAnalysisHistory, getLatestStockAnalysisSnapshots, mergeStockAnalysisHistory, } from '@/services/stock-analysis-history';
...
import { buildDailyMarketBrief, cacheDailyMarketBrief, getCachedDailyMarketBrief, shouldRefreshDailyBrief, } from '@/services/daily-market-brief';
...
export class DataLoaderManager {
    ...
    constructor(ctx, callbacks) {
        ...
        Object.defineProperty(this, "mapFlashCache", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "MAP_FLASH_COOLDOWN_MS", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 10 * 60 * 1000
        });
        Object.defineProperty(this, "applyTimeRangeFilterToNewsPanelsDebounced", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: debounce(() => {
        ...
        Object.defineProperty(this, "updateSearchIndex", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: () => { }
        });
        Object.defineProperty(this, "boundMarketWatchlistHandler", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "satellitePropagationCleanup", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "dailyBriefGeneration", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "dailyBriefFrameworkUnsubscribe", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "marketImplicationsFrameworkUnsubscribe", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "cachedSatRecs", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "digestBreaker", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: { state: 'closed', failures: 0, cooldownUntil: 0 }
        });
        Object.defineProperty(this, "digestRequestTimeoutMs", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 8000
        });
        Object.defineProperty(this, "digestBreakerCooldownMs", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 5 * 60 * 1000
        });
        Object.defineProperty(this, "persistedDigestMaxAgeMs", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 6 * 60 * 60 * 1000
        });
        Object.defineProperty(this, "perFeedFallbackCategoryFeedLimit", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 3
        });
        Object.defineProperty(this, "perFeedFallbackIntelFeedLimit", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 6
        });
        Object.defineProperty(this, "perFeedFallbackBatchSize", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 2
        });
        Object.defineProperty(this, "lastGoodDigest", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "imageryRetryTimer", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "lastWebcamBbox", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "lastWebcamFetchAt", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
    ...
    destroy() {
        ...
        if (this.boundMarketWatchlistHandler) {
            window.removeEventListener('wm-market-watchlist-changed', this.boundMarketWatchlistHandler);
    ...
    refreshCiiAndBrief(forceLocal = false) {
        this.ctx.panels['cii']?.refresh(forceLocal);
    ...
    async tryFetchDigest() {
        ...
        try {
            const resp = await fetch(toApiUrl(`/api/news/v1/list-feed-digest?variant=${SITE_VARIANT}&lang=${getCurrentLanguage()}`), { cache: 'no-cache', signal: AbortSignal.timeout(this.digestRequestTimeoutMs) });
            if (!resp.ok)
    ...
    async loadPersistedDigest() {
        try {
            const envelope = await getPersistentCache('digest:last-good');
            if (!envelope)
                return null;
    ...
    selectLimitedFeeds(feeds, maxFeeds) {
        if (feeds.length <= maxFeeds)
            return feeds;
        return feeds.slice(0, maxFeeds);
    }
    shouldShowIntelligenceNotifications() {
        return !this.ctx.isMobile && !!this.ctx.findingsBadge?.isPopupEnabled();
    }
    isPanelNearViewport(panelId, marginPx = 400) {
        const panel = this.ctx.panels[panelId];
        return panel?.isNearViewport?.(marginPx) ?? false;
    }
    isAnyPanelNearViewport(panelIds, marginPx = 400) {
        return panelIds.some((panelId) => this.isPanelNearViewport(panelId, marginPx));
    }
    async loadAllData(forceAll = false) {
        const runGuarded = async (name, fn) => {
            if (this.ctx.isDestroyed || this.ctx.inFlight.has(name))
                return;
        ...
        const shouldLoad = (id) => forceAll || this.isPanelNearViewport(id);
        const shouldLoadAny = (ids) => forceAll || this.isAnyPanelNearViewport(ids);
        const tasks = [
        ...
        if (SITE_VARIANT === 'full') {
            try {
                ...
                if (cached && cached.cii.length > 0) {
                    this.ctx.panels['cii']?.renderFromCached(cached);
        ...
        if (shouldLoad('economic'))
            tasks.push({ name: 'economicStress', task: runGuarded('economicStress', () => this.loadEconomicStress()) });
        ...
        if (SITE_VARIANT !== 'happy' && !isDesktopRuntime() && (this.ctx.mapLayers.iranAttacks || shouldLoadAny(['cii', 'strategic-risk', 'strategic-posture'])))
            tasks.push({ name: 'iranAttacks', task: runGuarded('iranAttacks', () => this.loadIranEvents()) });
        ...
        if (SITE_VARIANT !== 'happy' && this.ctx.mapLayers.satellites && this.ctx.map?.isGlobeMode?.())
            tasks.push({ name: 'satellites', task: runGuarded('satellites', () => this.loadSatellites()) });
        ...
        if (SITE_VARIANT !== 'happy') {
            tasks.push({ name: 'techReadiness', task: runGuarded('techReadiness', () => this.ctx.panels['tech-readiness']?.refresh()) });
        }
    ...
    async loadDataForLayer(layer) {
        if (this.ctx.isDestroyed || this.ctx.inFlight.has(layer))
            return;
    ...
    stopSatellitePropagation() {
        this.satellitePropagationCleanup?.();
        this.satellitePropagationCleanup = null;
    }
    loadImageryFootprints(retries = 2) {
        if (!this.ctx.mapLayers.satellites)
            return;
        if (this.ctx.map?.isGlobeMode())
            return;
    ...
    flashMapForNews(items) {
        if (!this.ctx.map || !this.ctx.initialLoadComplete)
            return;
        if (!getAiFlowSettings().mapNewsFlash)
            return;
        const now = Date.now();
    ...
    filterItemsByTimeRange(items, range = this.ctx.currentTimeRange) {
        if (range === 'all')
            return items;
    ...
    getTimeRangeLabel(range = this.ctx.currentTimeRange) {
        const labels = {
            '1h': 'the last hour',
            '6h': 'the last 6 hours',
    ...
    applyTimeRangeFilterDebounced() {
        this.applyTimeRangeFilterToNewsPanelsDebounced();
    }
    async loadNewsCategory(category, feeds, digest) {
        try {
            const panel = this.ctx.newsPanels[category];
            ...
            const scheduleRender = (partialItems) => {
                if (!panel)
                    return;
    ...
    async loadNews() {
        ...
        for (let i = 0; i < categories.length; i += categoryConcurrency) {
            ...
            const chunkResults = await Promise.allSettled(chunk.map(({ key, feeds }) => this.loadNewsCategory(key, feeds, digest)));
    ...
    async loadStockAnalysis() {
        const panel = this.ctx.panels['stock-analysis'];
        if (!panel)
            return;
        try {
    ...
    async loadStockBacktest() {
        const panel = this.ctx.panels['stock-backtest'];
        if (!panel)
            return;
        try {
            const targets = getStockAnalysisTargets();
    ...
    async loadMarkets() {
        try {
            ...
            const hydratedMarkets = getHydratedData('marketQuotes');
            let stocksResult;
            ...
            const commoditiesPanel = this.ctx.panels['commodities'];
            const energyPanel = this.ctx.panels['energy-complex'];
            const mapCommodity = (c) => ({ symbol: c.symbol, display: c.display, price: c.price, change: c.change, sparkline: c.sparkline });
            const energySymbols = new Set(['CL=F', 'BZ=F', 'NG=F']);
            const filterCommodityTape = (data) => data.filter((item) => item.symbol !== '^VIX' && !energySymbols.has(item.symbol));
            const filterEnergyTape = (data) => data.filter((item) => energySymbols.has(item.symbol));
            if (commoditiesPanel || energyPanel) {
                // Hydrate commodities from bootstrap (same pattern as sectors/markets)
                const hydratedCommodities = getHydratedData('commodityQuotes');
        ...
        try {
            const cryptoPanel = this.ctx.panels['crypto'];
        ...
        const cryptoHeatmapPanel = this.ctx.panels['crypto-heatmap'];
        const defiPanel = this.ctx.panels['defi-tokens'];
        const aiPanel = this.ctx.panels['ai-tokens'];
        const otherPanel = this.ctx.panels['other-tokens'];
    ...
    async loadDailyMarketBrief(force = false) {
        if (!hasPremiumAccess())
            return;
        ...
        try {
            ...
            if (this.dailyBriefGeneration !== gen)
                return;
            if (!brief.available) {
                if (!cached?.available) {
    ...
    async _collectRegimeContext() {
        try {
            const hydrated = getHydratedData('fearGreedIndex');
            if (hydrated && !hydrated.unavailable && Number(hydrated.compositeScore) > 0) {
                const comp = hydrated.composite;
                const cats = (hydrated.categories ?? {});
                const hdr = (hydrated.headerMetrics ?? {});
                return {
            ...
            if (resp.unavailable || resp.compositeScore <= 0)
                return undefined;
            return {
    ...
    async _collectYieldCurveContext() {
        try {
    ...
    async loadMarketImplications() {
        if (!hasPremiumAccess())
            return;
        if (this.ctx.isDestroyed || this.ctx.inFlight.has('marketImplications'))
            return;
    ...
    async loadPredictions() {
        try {
            ...
            this.ctx.panels['polymarket']?.renderPredictions(predictions);
    ...
    async loadForecasts() {
        try {
            const hydrated = getHydratedData('forecasts');
    ...
    async loadTechEvents() {
        ...
        try {
            ...
            const hydrated = getHydratedData('techEvents');
            ...
            if (!events?.length) {
                ...
                events = events.filter(e => e.type === 'conference' &&
                    e.coords && !e.coords.virtual &&
                    new Date(e.startDate) <= cutoff).slice(0, 50);
            }
            const now = new Date();
            ...
            if (SITE_VARIANT === 'tech' && this.ctx.searchModal) {
                this.ctx.searchModal.registerSource('techevent', mapEvents.map((e) => ({
                    id: e.id,
                    title: e.title,
    ...
    async loadIntelligenceSignals() {
        ...
        tasks.push((async () => {
            try {
                ...
                this.ctx.panels['internet-disruptions']?.setOutages(outages);
                fetchTrafficAnomalies().then(r => {
                    this.ctx.map?.setTrafficAnomalies(r.anomalies);
                    this.ctx.panels['internet-disruptions']?.setAnomalies(r.anomalies);
                }).catch(() => { });
                fetchDdosAttacks().then(r => {
                    this.ctx.map?.setDdosLocations(r.topTargetLocations ?? []);
                    this.ctx.panels['internet-disruptions']?.setDdos(r);
                }).catch(() => { });
            }
            catch (error) {
        ...
        const hydratedUcdp = getHydratedData('ucdpEvents');
        tasks.push((async () => {
            try {
        ...
        tasks.push((async () => {
            try {
                ...
                fetchUSNIFleetReport().then((report) => {
                    if (report)
                        this.ctx.intelligenceCache.usniFleet = report;
                }).catch(() => { });
        ...
        tasks.push((async () => {
            try {
                ...
                this.ctx.panels['ucdp-events']?.setEvents(events);
        ...
        try {
            const ucdpEvts = this.ctx.panels['ucdp-events']?.getEvents?.() || [];
            const events = [
                ...(this.ctx.intelligenceCache.protests?.events || []).slice(0, 10).map(e => ({
                    id: e.id, lat: e.lat, lon: e.lon, type: 'conflict', name: e.title || 'Protest',
                })),
                ...ucdpEvts.slice(0, 10).map(e => ({
                    id: e.id, lat: e.latitude, lon: e.longitude, type: e.type_of_violence, name: `${e.side_a} vs ${e.side_b}`,
                })),
            ];
            if (events.length > 0) {
    ...
    async loadWebcams() {
        ...
        try {
            ...
            const now = Date.now();
            if (now - this.lastWebcamFetchAt < 1000)
                return;
            ...
            if (this.lastWebcamBbox && this.lastWebcamBbox.zoom === zoom) {
                ...
                if (overlapArea / currentArea > 0.8)
                    return;
            }
    ...
    async loadCachedPosturesForBanner() {
        try {
            ...
            if (data && data.postures.length > 0) {
                ...
                const posturePanel = this.ctx.panels['strategic-posture'];
    ...
    async loadTradePolicy() {
        const tradePanel = this.ctx.panels['trade-policy'];
        if (!tradePanel)
            return;
        try {
            ...
            if (r)
                tradePanel.updateRestrictions(r);
            if (ta)
                tradePanel.updateTariffs(ta);
            if (fl)
                tradePanel.updateFlows(fl);
            if (ba)
                tradePanel.updateBarriers(ba);
            if (rev)
                tradePanel.updateRevenue(rev);
            if (ct)
                tradePanel.updateComtradeFlows(ct);
    ...
    async loadSupplyChain() {
        const scPanel = this.ctx.panels['supply-chain'];
        if (!scPanel)
            return;
        try {
            ...
            if (shippingData)
                scPanel.updateShippingRates(shippingData);
            if (chokepointData)
                scPanel.updateChokepointStatus(chokepointData);
            if (chokepointData)
                this.ctx.map?.setChokepointData(chokepointData);
            if (mineralsData)
                scPanel.updateCriticalMinerals(mineralsData);
            if (stressData)
                scPanel.updateShippingStress(stressData);
    ...
    async loadDiseaseOutbreaks() {
        try {
            const data = await fetchDiseaseOutbreaks();
            if (data.outbreaks?.length) {
                const panel = this.ctx.panels['disease-outbreaks'];
    ...
    async loadSocialVelocity() {
        try {
            const data = await fetchSocialVelocity();
            if (data.posts?.length) {
                const panel = this.ctx.panels['social-velocity'];
    ...
    async loadEconomicStress() {
        try {
            const economicPanel = this.ctx.panels['economic'];
            if (!economicPanel)
                return;
            const hydrated = getHydratedData('economicStress');
    ...
    updateMonitorResults() {
        const monitorPanel = this.ctx.panels['monitors'];
    ...
    async runCorrelationAnalysis() {
        try {
            ...
            const signals = await analysisWorker.analyzeCorrelations(this.ctx.latestClusters, this.ctx.latestPredictions, this.ctx.latestMarkets);
            let geoSignals = [];
            if (!isInLearningMode()) {
    ...
    async loadFirmsData() {
        try {
            ...
            if (totalCount > 0) {
                ...
                this.ctx.panels['satellite-fires']?.update(stats, totalCount);
                dataFreshness.recordUpdate('firms', totalCount);
            }
            else {
                ingestSatelliteFiresForCII([]);
                this.refreshCiiAndBrief();
                this.ctx.panels['satellite-fires']?.update([], 0);
            }
    ...
    async hydrateHappyPanelsFromCache() {
        try {
            const entry = await getPersistentCache(DataLoaderManager.HAPPY_ITEMS_CACHE_KEY);
            if (!entry || !entry.data || entry.data.length === 0)
                return;
            if (Date.now() - entry.updatedAt > 24 * 60 * 60 * 1000)
                return;
            const items = entry.data.map(item => ({
                ...item,
            ...
            this.callPanel('breakthroughs', 'setItems', items.filter(item => scienceSources.includes(item.source) || item.happyCategory === 'science-health'));
            this.callPanel('spotlight', 'setHeroStory', items.filter(item => item.happyCategory === 'humanity-kindness')
                .sort((a, b) => b.pubDate.getTime() - a.pubDate.getTime())[0]);
            this.callPanel('digest', 'setStories', [...items].sort((a, b) => b.pubDate.getTime() - a.pubDate.getTime()).slice(0, 5));
    ...
    async loadHappySupplementaryAndRender() {
        ...
        try {
            ...
            const gdeltItems = gdeltTopics.flatMap(topic => topic.articles.map(article => ({
                source: 'GDELT',
                title: article.title,
                link: article.url,
        ...
        const scienceSources = ['GNN Science', 'ScienceDaily', 'Nature News', 'Live Science', 'New Scientist', 'Singularity Hub', 'Human Progress', 'Greater Good (Berkeley)'];
        const scienceItems = this.ctx.happyAllItems.filter(item => scienceSources.includes(item.source) || item.happyCategory === 'science-health');
        ...
        setPersistentCache(DataLoaderManager.HAPPY_ITEMS_CACHE_KEY, this.ctx.happyAllItems.map(item => ({ ...item, pubDate: item.pubDate.getTime() }))).catch(() => { });
    }
    async loadPositiveEvents() {
        const hydrated = getHydratedData('positiveGeoEvents');
        let gdeltEvents;
        if (hydrated?.events?.length) {
            gdeltEvents = hydrated.events.map(e => ({
                lat: e.latitude, lon: e.longitude, name: e.name,
                category: (e.category || 'humanity-kindness'),
        ...
        const rssEvents = geocodePositiveNewsItems(this.ctx.happyAllItems.map(item => ({
            title: item.title,
    ...
    loadKindnessData() {
        const kindnessItems = fetchKindnessData(this.ctx.happyAllItems.map(item => ({
            title: item.title,
    ...
    async loadTelegramIntel() {
        if (isDesktopRuntime() && !hasPremiumAccess())
            return;
        try {
    ...
    async loadThermalEscalations() {
        try {
            ...
            dataFreshness.recordUpdate('thermal-escalation', result.clusters.length);
        }
        catch (error) {
    ...
    async loadCrossSourceSignals() {
        try {
            const result = await fetchCrossSourceSignals();
            this.callPanel('cross-source-signals', 'setData', result);
            dataFreshness.recordUpdate('cross-source-signals', result.signals?.length ?? 0);
        }
        catch (error) {
...
Object.defineProperty(DataLoaderManager, "HAPPY_ITEMS_CACHE_KEY", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: 'happy-all-items'
});
