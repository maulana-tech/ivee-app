...
export class SearchManager {
    ...
    setupSearchModal() {
        ...
        this.ctx.searchModal.setActivePanels(Object.entries(this.ctx.panelSettings).filter(([, v]) => v.enabled).map(([k]) => k));
        ...
        // so mid-session sign-ins get the feature without a page reload.
        {
            this.ctx.searchModal.setOnFlightSearch((callsign) => {
                if (!isProUser() && getAuthState().user?.role !== 'pro')
                    return;
                fetchAircraftPositions({ callsign }).then((positions) => {
                    ...
                    const items = [...seen.values()].map(p => {
                        ...
                        return {
                            ...
                            data: { kind: 'adsb', lat: p.lat, lon: p.lon, layer: 'flights' },
                        };
                    });
    ...
    handleSearchResult(result) {
        ...
        switch (result.type) {
            ...
            case 'flight': {
                const { lat, lon, layer } = result.data;
    ...
    handleCommand(cmd) {
        ...
        switch (category) {
            case 'nav':
                ...
                break;
            case 'layers': {
                const allowed = getAllowedLayerKeys((SITE_VARIANT || 'full'));
                if (action === 'all') {
                    for (const key of Object.keys(this.ctx.mapLayers)) {
                        this.ctx.mapLayers[key] = allowed.has(key);
                    }
                }
                else if (action === 'none') {
                    for (const key of Object.keys(this.ctx.mapLayers))
                        this.ctx.mapLayers[key] = false;
                }
                else {
            ...
            case 'layer': {
                const layerKey = (LAYER_KEY_MAP[action] || action);
                if (!(layerKey in this.ctx.mapLayers))
                    return;
                const variantAllowed = getAllowedLayerKeys((SITE_VARIANT || 'full'));
                if (!variantAllowed.has(layerKey))
                    return;
            ...
            case 'view':
                if (action === 'dark' || action === 'light') {
                    ...
                    const layerKey = 'resilienceScore';
                    const variantAllowed = getAllowedLayerKeys((SITE_VARIANT || 'full'));
                    if (!variantAllowed.has(layerKey))
                        break;
                    let newValue = !this.ctx.mapLayers[layerKey];
                    if (newValue && !this.ctx.map?.isDeckGLActive?.())
                        newValue = false;
                    this.ctx.mapLayers[layerKey] = newValue;
                    saveToStorage(STORAGE_KEYS.mapLayers, this.ctx.mapLayers);
                    if (newValue) {
                ...
                break;
            case 'time':
                this.ctx.map?.setTimeRange(action);
                break;
            case 'country': {
    ...
    updateFlightSource(adsb, military) {
        if (!this.ctx.searchModal || !isProUser())
            return;
        const items = [
            ...adsb.map(p => {
                const fl = Number.isFinite(p.altitudeFt) ? Math.round(p.altitudeFt / 100) : null;
                ...
                return {
                    ...
                    subtitle: f.onGround
                        ...
                        : fl !== null
                            ...
                            : t('modals.search.flightMilitaryOnGround', { type: f.aircraftType }),
                    data: { kind: 'military', lat: f.lat, lon: f.lon, layer: 'military' },
                };
            }),
        ];
        this.ctx.searchModal.registerSource('flight', items);
    ...
    buildCountrySearchItems() {
        const panelScores = this.ctx.panels.cii?.getScores() ?? [];
