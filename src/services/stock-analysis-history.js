...
async function getTargetSymbols(limit) {
...
export function mergeStockAnalysisHistory(existing, incoming, maxSnapshotsPerSymbol = MAX_SNAPSHOTS_PER_SYMBOL) {
    const next = { ...existing };
    for (const snapshot of incoming) {
        if (!snapshot?.symbol || !snapshot.available)
            continue;
...
export function getLatestStockAnalysisSnapshots(history, limit = DEFAULT_LIMIT) {
...
export function hasFreshStockAnalysisHistory(history, symbols, maxAgeMs = STOCK_ANALYSIS_FRESH_MS) {
    if (symbols.length === 0)
        return false;
    const now = Date.now();
...
export function getMissingOrStaleStockAnalysisSymbols(history, symbols, maxAgeMs = STOCK_ANALYSIS_FRESH_MS) {
    const now = Date.now();
...
export async function fetchStockAnalysisHistory(limit = DEFAULT_LIMIT, limitPerSymbol = DEFAULT_LIMIT_PER_SYMBOL) {
