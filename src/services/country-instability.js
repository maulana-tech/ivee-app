...
export function setIntelligenceSignalsLoaded() {
    intelligenceSignalsLoaded = true;
}
export function hasIntelligenceSignalsLoaded() {
    return intelligenceSignalsLoaded;
}
export function hasAnyIntelligenceData() {
...
export function setHasCachedScores(hasScores) {
...
export function isInLearningMode() {
...
export function resetIngestStats() {
    processedCount = 0;
    unmappedCount = 0;
}
...
export function ingestProtestsForCII(events) {
...
export function ingestConflictsForCII(events) {
    for (const [, data] of countryDataMap)
        data.conflicts = [];
    for (const e of events) {
        processedCount++;
...
export function ingestUcdpForCII(classifications) {
...
export function ingestHapiForCII(summaries) {
...
export function ingestDisplacementForCII(countries) {
    for (const data of countryDataMap.values()) {
...
export function ingestClimateForCII(anomalies) {
    for (const data of countryDataMap.values()) {
        data.climateStress = 0;
    }
...
export function resetHotspotActivity() {
    hotspotActivityMap.clear();
}
...
export function ingestMilitaryForCII(flights, vessels) {
    for (const [, data] of countryDataMap) {
...
export function ingestNewsForCII(events) {
    for (const e of events) {
...
export function ingestOutagesForCII(outages) {
    for (const [, data] of countryDataMap)
        data.outages = [];
...
export function ingestOrefForCII(alertCount, historyCount24h) {
...
export function ingestAviationForCII(alerts) {
    for (const [, data] of countryDataMap)
        data.aviationDisruptions = [];
...
const ADVISORY_LEVEL_RANK = { 'do-not-travel': 4, 'reconsider': 3, 'caution': 2, 'normal': 1, 'info': 0 };
export function ingestAdvisoriesForCII(advisories) {
    for (const data of countryDataMap.values()) {
        data.advisoryMaxLevel = null;
        data.advisoryCount = 0;
        data.advisorySources = new Set();
    }
    const travelAdvisories = advisories.filter(a => a.country && TRAVEL_ADVISORY_SOURCES.has(a.sourceCountry) && a.level && a.level !== 'info');
    for (const a of travelAdvisories) {
        ...
        if (newRank > currentRank)
            data.advisoryMaxLevel = a.level;
    }
}
function getAdvisoryBoost(data) {
    ...
    if (data.advisorySources.size >= 3)
        boost += 5;
    else if (data.advisorySources.size >= 2)
        boost += 3;
    return boost;
}
function getAdvisoryFloor(data) {
    if (data.advisoryMaxLevel === 'do-not-travel')
        return 60;
    if (data.advisoryMaxLevel === 'reconsider')
        return 50;
    return 0;
}
function getSupplementalSignalBoost(data) {
    const aisBoost = Math.min(10, data.aisDisruptionHighCount * 2.5 + data.aisDisruptionElevatedCount * 1.5 + data.aisDisruptionLowCount * 0.5);
    const fireBoost = Math.min(8, data.satelliteFireHighCount * 1.5 + Math.min(20, data.satelliteFireCount) * 0.25);
    const cyberBoost = Math.min(12, data.cyberThreatCriticalCount * 3 + data.cyberThreatHighCount * 1.8 + data.cyberThreatMediumCount * 0.9);
    const temporalBoost = Math.min(6, data.temporalAnomalyCriticalCount * 2 + data.temporalAnomalyCount * 0.75);
    return aisBoost + fireBoost + cyberBoost + temporalBoost;
}
const h3CountryCache = new Map();
export function ingestGpsJammingForCII(hexes) {
    for (const [, data] of countryDataMap) {
        data.gpsJammingHighCount = 0;
        data.gpsJammingMediumCount = 0;
    }
    for (const hex of hexes) {
        ...
        const data = countryDataMap.get(code);
        if (hex.level === 'high')
            data.gpsJammingHighCount++;
        else
            data.gpsJammingMediumCount++;
    }
}
function resolveCountryForSignal(countryHint, lat, lon) {
    if (countryHint) {
...
export function ingestAisDisruptionsForCII(events) {
    for (const [, data] of countryDataMap) {
        data.aisDisruptionHighCount = 0;
        data.aisDisruptionElevatedCount = 0;
        data.aisDisruptionLowCount = 0;
    ...
    for (const e of events) {
        ...
        const data = countryDataMap.get(code);
        if (e.severity === 'high')
            data.aisDisruptionHighCount++;
        else if (e.severity === 'elevated')
            data.aisDisruptionElevatedCount++;
        else
            data.aisDisruptionLowCount++;
    }
}
...
export function ingestCyberThreatsForCII(threats) {
    for (const [, data] of countryDataMap) {
    ...
    for (const threat of threats) {
        ...
        if (threat.severity === 'critical')
            data.cyberThreatCriticalCount++;
        else if (threat.severity === 'high')
            data.cyberThreatHighCount++;
        else if (threat.severity === 'medium')
            data.cyberThreatMediumCount++;
    }
}
export function ingestTemporalAnomaliesForCII(anomalies) {
    for (const [, data] of countryDataMap) {
    ...
    for (const anomaly of anomalies) {
        ...
        if (anomaly.severity === 'critical')
            data.temporalAnomalyCriticalCount++;
    }
}
const EARTHQUAKE_LOOKBACK_MS = 7 * 24 * 60 * 60 * 1000;
export function ingestEarthquakesForCII(earthquakes, now = Date.now()) {
    for (const [, data] of countryDataMap) {
    ...
    for (const eq of earthquakes) {
        ...
        const data = countryDataMap.get(code);
        if (eq.magnitude >= 7.5)
            data.earthquakeSevereCount++;
        else if (eq.magnitude >= 6.5)
            data.earthquakeMajorCount++;
        else
            data.earthquakeSignificantCount++;
    }
}
export function ingestSanctionsForCII(countries) {
    for (const [, data] of countryDataMap) {
...
function getEarthquakeBoost(data) {
...
export function getTopUnstableCountries(limit = 10) {
    return calculateCII().slice(0, limit);
}
export function getCountryScore(code) {
    const data = countryDataMap.get(code);
    if (!data)
        return null;
    const baselineRisk = CURATED_COUNTRIES[code]?.baselineRisk ?? DEFAULT_BASELINE_RISK;
