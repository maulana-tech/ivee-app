...
export function updateAlertSettings(partial) {
...
export function checkBatchForBreakingAlerts(items) {
    ...
    for (const item of items) {
        ...
        if (phase === 'sustained' || phase === 'fading')
            continue;
        // Items below the importance threshold are too low-signal for the banner.
        if (item.importanceScore !== undefined && item.importanceScore < IMPORTANCE_SCORE_MIN)
            continue;
        const isBetter = !best
...
export function dispatchOrefBreakingAlert(alerts) {
    const settings = getAlertSettings();
    if (!settings.enabled || !alerts.length)
        return;
...
export function initBreakingNewsAlerts() {
...
export function destroyBreakingNewsAlerts() {
    if (storageListener) {
