...
const AI_CLASSIFY_MAX_PER_WINDOW = SITE_VARIANT === 'finance' ? 40 : SITE_VARIANT === 'tech' ? 60 : 80;
export const AI_CLASSIFY_MAX_PER_FEED = SITE_VARIANT === 'finance' ? 2 : SITE_VARIANT === 'tech' ? 2 : 3;
const aiRecentlyQueued = new Map();
const aiDispatches = [];
function toAiKey(title) {
    return title.trim().toLowerCase().replace(/\s+/g, ' ');
}
export function canQueueAiClassification(title) {
    const now = Date.now();
