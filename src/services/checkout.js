...
const APP_CHECKOUT_BASE_URL = 'https://ivee.app/';
 ...
 */
export function initCheckoutOverlay(onSuccess) {
    if (initialized)
        return;
    if (onSuccess) {
...
function loadPendingCheckoutIntent() {
    try {
...
function savePendingCheckoutIntent(intent) {
    try {
...
export function buildCheckoutLaunchUrl(productId, options) {
...
export async function resumePendingCheckout(options) {
    ...
    const success = await startCheckout(intent.productId, {
 ...
 */
export async function startCheckout(productId, options, behavior) {
