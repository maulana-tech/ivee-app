import { startSmartPollLoop, toApiUrl } from '@/services/runtime';
...
let updateCallbacks = [];
let locationTranslator = null;
let locationMapPromise = null;
async function ensureLocationMapLoaded() {
    if (locationTranslator)
        return;
...
function parseTranslationResponse(raw, alerts) {
    ...
    for (const alert of alerts) {
        ...
        if (title === null && areas === null && desc === null)
            continue;
        const entry = {
...
function translateFields(alert) {
    return {
        ...alert,
...
export function onOrefAlertsUpdate(cb) {
    updateCallbacks.push(cb);
}
export function startOrefPolling() {
    if (pollingLoop?.isActive())
        return;
...
export function stopOrefPolling() {
    pollingLoop?.stop();
    pollingLoop = null;
    updateCallbacks = [];
}
