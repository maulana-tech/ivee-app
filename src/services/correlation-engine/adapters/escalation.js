...
export const escalationAdapter = {
    domain: 'escalation',
    ...
    collectSignals(ctx) {
        ...
        for (const c of clusters) {
            ...
            if (age > windowMs)
                continue;
            if (!ESCALATION_KEYWORDS.test(c.primaryTitle))
                continue;
        ...
        const conflictCountries = new Set(signals.filter(s => s.type === 'conflict_event').map(s => s.country).filter(Boolean));
