...
export const economicAdapter = {
    ...
    collectSignals(ctx) {
        ...
        for (const c of clusters) {
            ...
            if (age > windowMs)
                continue;
            if (!SANCTIONS_KEYWORDS.test(c.primaryTitle))
                continue;
    ...
    generateTitle(cluster, context) {
        ...
        if (types.has('commodity_spike')) {
            ...
            const names = spikes
                .map(s => s.rawData?.display
                ?? s.rawData?.symbol
                ?? s.label.split(' ')[0])
                .slice(0, 2);
            const pctParts = spikes
                .map(s => {
                const change = s.rawData?.change;
...
function extractMentionedEntities(labels) {
...
function displayEntity(key) {
    if (!key)
        return '';
    if (GENERIC_ENTITY_KEYS.has(key))
        return '';
