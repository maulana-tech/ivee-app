...
export const disasterAdapter = {
    ...
    collectSignals(ctx) {
        ...
        const conflictCountries = new Set((cache.protests?.events ?? [])
            .filter(p => {
        ...
        for (const o of outages) {
            ...
            if (age > windowMs)
                continue;
            if (o.country && conflictCountries.has(o.country))
                continue;
    ...
    generateTitle(cluster) {
        ...
        if (types.has('earthquake')) {
            const maxMag = Math.max(...cluster
                .filter(s => s.type === 'earthquake')
                .map(s => s.rawData?.magnitude ?? 0));
            parts.push(`M${maxMag.toFixed(1)} seismic`);
        }
        if (types.has('infra_outage'))
            parts.push('infra disruption');
