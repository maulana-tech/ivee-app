...
export const militaryAdapter = {
    ...
    generateTitle(cluster) {
        ...
        const flightTypes = new Set(cluster
            .filter(s => s.type === 'military_flight')
            .map(s => s.rawData?.aircraftType)
            .filter(Boolean));
        ...
        if (hasFlights)
            return `Military flight cluster \u2014 ${countryLabel}`;
        if (hasVessels)
            return `Naval vessel concentration \u2014 ${countryLabel}`;
