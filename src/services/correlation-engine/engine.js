...
export class CorrelationEngine {
    constructor() {
        ...
        Object.defineProperty(this, "previousClusters", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "llmCache", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "intelligenceClient", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "llmInFlight", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
    ...
    async run(ctx) {
        ...
        try {
            ...
            for (const adapter of this.adapters) {
                ...
                this.previousClusters.set(adapter.domain, withTrend.map(c => c.state));
    ...
    // ── Clustering ──────────────────────────────────────────────
    clusterSignals(signals, adapter) {
        if (signals.length === 0)
            return [];
    ...
    clusterByEntity(signals) {
        ...
        for (const s of signals) {
            ...
            if (!matchedKey)
                continue; // drop unmatched signals to avoid false convergence
    ...
    clusterByProximity(signals, radiusKm) {
    ...
    // ── Scoring ─────────────────────────────────────────────────
    scoreClusters(clusters, adapter) {
    ...
    // ── LLM Assessment ─────────────────────────────────────────
    queueLlmAssessments(cards, adapter) {
        if (!hasPremiumAccess())
            return;
        const pending = [];
        for (const card of cards) {
            if (card.score < LLM_SCORE_THRESHOLD)
                continue;
        ...
        for (const { card, cacheKey } of pending) {
            if (this.llmInFlight >= LLM_MAX_CONCURRENT)
                break;
            this.llmInFlight++;
    ...
    async fetchAssessment(card, adapter, cacheKey) {
        try {
