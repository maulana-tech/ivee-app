...
const CONFIG_KEY = 'ivee-trending-config-v1';
...
export async function extractEntitiesWithML(text) {
...
async function handleSpike(spike, config) {
    ...
    try {
        ...
        if (config.autoSummarize && headlines.length >= 2 && canRunAutoSummary(now)) {
            ...
            const summary = await generateSummary(headlines, undefined, `Breaking: "${spike.term}" mentioned ${spike.count}x in ${windowHours}h (${multiplierText})`);
            if (summary?.summary) {
...
async function enrichWithMLEntities(headlines, ingestedAt) {
    if (headlines.length === 0 || !mlWorker.isAvailable)
        return;
    try {
...
export function ingestHeadlines(headlines) {
...
export function drainTrendingSignals() {
    if (pendingSignals.length === 0)
        return [];
    return pendingSignals.splice(0, pendingSignals.length);
}
export function getTrendingConfig() {
    return { ...readConfig() };
}
export function updateTrendingConfig(update) {
...
export function suppressTrendingTerm(term) {
    const config = readConfig();
...
export function unsuppressTrendingTerm(term) {
    const config = readConfig();
...
export function getTrackedTermCount() {
