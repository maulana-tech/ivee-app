...
export const POSITIVE_GDELT_TOPICS = [
    {
...
export function getIntelTopics() {
...
const gdeltBreaker = createCircuitBreaker({ name: 'GDELT Intelligence', cacheTtlMs: 10 * 60 * 1000, persistCache: true });
const positiveGdeltBreaker = createCircuitBreaker({ name: 'GDELT Positive', cacheTtlMs: 10 * 60 * 1000, persistCache: true });
...
export async function fetchTopicTimeline(topicId) {
    ...
    try {
        ...
        if (resp.error || (resp.tone.length === 0 && resp.vol.length === 0))
            return null;
        const data = { tone: resp.tone, vol: resp.vol, fetchedAt: resp.fetchedAt };
...
export async function fetchGdeltArticles(query, maxrecords = 10, timespan = '24h') {
...
export async function fetchAllTopicIntelligence() {
    const results = await Promise.allSettled(INTEL_TOPICS.map(topic => fetchTopicIntelligence(topic)));
    return results
...
export async function fetchPositiveGdeltArticles(query, toneFilter = 'tone>5', sort = 'ToneDesc', maxrecords = 15, timespan = '72h') {
...
export async function fetchPositiveTopicIntelligence(topic) {
...
export async function fetchAllPositiveTopicIntelligence() {
    const results = await Promise.allSettled(POSITIVE_GDELT_TOPICS.map(topic => fetchPositiveTopicIntelligence(topic)));
    return results
        .filter((r) => r.status === 'fulfilled')
        .map(r => r.value);
}
