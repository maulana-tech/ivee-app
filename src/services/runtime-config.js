...
const TOGGLES_STORAGE_KEY = 'ivee-runtime-feature-toggles';
function getSidecarEnvUpdateUrl() {
    return `${getApiBaseUrl()}/api/local-env-update`;
}
function getSidecarEnvUpdateBatchUrl() {
...
export const secretsReady = new Promise(r => { secretsReadyResolve = r; });
if (!isDesktopRuntime())
    secretsReadyResolve();
...
function seedSecretsFromEnvironment() {
    if (isDesktopRuntime())
        return;
    const keys = new Set(RUNTIME_FEATURES.flatMap(feature => feature.requiredSecrets));
...
export function getSecretState(key) {
...
export async function setSecretValue(key, value) {
    ...
    if (sanitized) {
        await invokeTauri('set_secret', { key, value: sanitized });
...
async function getLocalApiToken() {
    if (!localApiTokenPromise) {
        localApiTokenPromise = invokeTauri('get_local_api_token')
...
export async function verifySecretWithApi(key, value, context = {}) {
...
export async function loadDesktopSecrets() {
    if (!isDesktopRuntime())
        return;
    try {
        const allSecrets = await invokeTauri('get_all_secrets');
        const entries = [];
        ...
        if (entries.length > 0) {
            try {
                ...
                await Promise.allSettled(entries.map(({ key, value }) => pushSecretToSidecar(key, value).catch((error) => {
                    console.warn(`[runtime-config] Failed to sync ${key} to sidecar`, error);
                })));
            }
        }
...
async function pushSecretBatchToSidecar(entries) {
    const headers = new Headers({ 'Content-Type': 'application/json' });
    const token = await getLocalApiToken();
    if (token) {
        headers.set('Authorization', `Bearer ${token}`);
