...
export function loadFrameworkLibrary() {
    const imported = loadFromStorage(LIBRARY_KEY, []);
    return [...BUILT_IN_FRAMEWORKS, ...imported];
}
export function saveImportedFramework(fw) {
    const imported = loadFromStorage(LIBRARY_KEY, []);
    ...
    const newFw = { ...fw, isBuiltIn: false, createdAt: Date.now() };
    saveToStorage(LIBRARY_KEY, [...imported, newFw]);
    _activeCache.clear();
}
export function deleteImportedFramework(id) {
    if (BUILT_IN_FRAMEWORKS.some(f => f.id === id))
        return;
    const imported = loadFromStorage(LIBRARY_KEY, []);
    saveToStorage(LIBRARY_KEY, imported.filter(f => f.id !== id));
    _activeCache.clear();
}
export function renameImportedFramework(id, name) {
    if (BUILT_IN_FRAMEWORKS.some(f => f.id === id))
        return;
    const imported = loadFromStorage(LIBRARY_KEY, []);
...
export function getActiveFrameworkForPanel(panelId) {
    if (!hasPremiumAccess())
        return null;
    if (_activeCache.has(panelId))
        return _activeCache.get(panelId);
    const selections = loadFromStorage(PANEL_KEY, {});
...
export function subscribeFrameworkChange(panelId, cb) {
    const listener = (e) => {
        const evt = e;
        if (evt.detail.panelId === panelId)
            cb();
    };
