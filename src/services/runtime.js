...
export async function resolveLocalApiPort() {
...
export function detectDesktopRuntime(probe) {
...
function isIveeWebHost(hostname) {
    return hostname === 'ivee.app'
        || hostname === 'www.ivee.app'
        || hostname.endsWith('.ivee.app');
}
export function getConfiguredWebApiBaseUrl() {
    if (WS_API_URL) {
...
export function toRuntimeUrl(path) {
    if (!path.startsWith('/')) {
        return path;
    }
...
export class VisibilityHub {
    ...
    ensureListening() {
        if (this.listening || !hasVisibilityApi())
            return;
        this.handler = () => {
    ...
    stopListening() {
        if (!this.listening || !this.handler)
            return;
...
export function startSmartPollLoop(poll, opts) {
    const intervalMs = Math.max(1000, Math.round(opts.intervalMs));
    ...
    const minIntervalMs = Math.max(250, opts.minIntervalMs ?? 1000);
...
export async function waitForSidecarReady(timeoutMs = 3000) {
    const baseUrl = getApiBaseUrl();
    if (!baseUrl)
        return false;
            ...
            if (response.status === 401 && localApiToken && Date.now() > authRetryCooldownUntil) {
                ...
                if (localApiToken) {
                    ...
                    if (response.status === 401) {
                        authRetryCooldownUntil = Date.now() + 60000;
                        if (debug)
                            console.log(`[fetch] auth retry failed, suppressing retries for 60s`);
                    }
                    else {
...
export function installWebApiRedirect() {
    ...
    const nativeFetch = window.fetch.bind(window);
    const shouldRedirectPath = (pathWithQuery) => pathWithQuery.startsWith('/api/');
    /**
     ...
     */
    const enrichInitForPremium = async (pathWithQuery, init) => {
        const path = pathWithQuery.split('?')[0] ?? pathWithQuery;
        if (!WEB_PREMIUM_API_PATHS.has(path))
            return init;
        const headers = new Headers(init?.headers);
        // Don't overwrite existing auth headers
        if (headers.has('Authorization') || headers.has('X-Ivee-Key'))
            return init;
    ...
    if (hasRedirect) {
        ...
        const shouldFallbackToOrigin = (status) => (status === 404 || status === 405 || status === 501 || status === 502 || status === 503);
        const fetchWithRedirectFallback = async (redirectedInput, originalInput, originalInit) => {
            try {
        ...
        window.fetch = async (input, init) => {
            ...
            if (input instanceof Request) {
                ...
                if (u.origin === window.location.origin && shouldRedirectPath(pathAndSearch)) {
                    const enriched = await enrichInitForPremium(pathAndSearch, init);
                    return fetchWithRedirectFallback(new Request(`${API_BASE}${pathAndSearch}`, input), input.clone(), enriched);
                }
                // Request object already targeting the API base.
                if (u.origin === API_BASE && pathAndSearch.startsWith('/api/')) {
                    const enriched = await enrichInitForPremium(pathAndSearch, init);
                    if (enriched)
                        return nativeFetch(new Request(input, enriched));
                }
            }
            return nativeFetch(input, init);
        };
