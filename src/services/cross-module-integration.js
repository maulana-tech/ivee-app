import { getLocationName } from './geo-convergence';
import { getLatestSanctionsPressure } from './sanctions-pressure';
import { getLatestRadiationWatch } from './radiation';
...
function getPriorityFromSanctions(data) {
    const leadEntryCount = data.countries[0]?.entryCount ?? 0;
    if (data.newEntryCount >= 10)
        return 'critical';
    if (data.newEntryCount >= 3 || leadEntryCount >= 60)
        return 'high';
    if (data.newEntryCount >= 1 || leadEntryCount >= 25)
        return 'medium';
    return 'low';
}
function getPriorityFromRadiation(observation, spikeCount) {
    let score = 0;
    if (observation.severity === 'spike')
        score += 4;
    else if (observation.severity === 'elevated')
        score += 2;
    if (observation.corroborated)
        score += 2;
    if (observation.confidence === 'high')
        score += 2;
    else if (observation.confidence === 'medium')
        score += 1;
    if (observation.conflictingSources)
        score -= 2;
    if (observation.convertedFromCpm)
        score -= 1;
    if (spikeCount > 1 && observation.corroborated)
        score += 1;
    if (score >= 7)
        return 'critical';
    if (score >= 4)
...
export function createConvergenceAlert(convergence) {
    const alertId = `conv-${convergence.cellId}`;
    const alert = buildConvergenceAlert(convergence, alertId);
    return addAndMergeAlert(alert);
}
export function createCIIAlert(country, countryName, previousScore, currentScore, level, driver) {
...
export function createCascadeAlert(cascade) {
...
function createSanctionsAlert() {
    ...
    if (!leadCountry)
        return null;
    if (pressure.newEntryCount === 0 && leadCountry.entryCount < 25)
        return null;
...
function getRadiationRank(observation) {
...
function generateCompositeTitle(a, b) {
    ...
    if (a.components.convergence || b.components.convergence) {
        if (a.components.sanctions || b.components.sanctions) {
            ...
            if (sanctions)
                return `Sanctions pressure: ${sanctions.countryName}`;
        }
...
export function checkCIIChanges() {
    const newAlerts = [];
...
export function calculateStrategicRiskOverview(convergenceAlerts, theaterPostures, breakingAlertScore, theaterStaleFactor) {
    ...
    const sanctionsScore = sanctionsPressure
        ? Math.min(10, sanctionsPressure.newEntryCount * 2 +
    ...
    const radiationScore = radiationWatch
        ? Math.min(12, radiationWatch.summary.spikeCount * 4 +
    ...
    if (theaterPostures && theaterPostures.length > 0) {
        for (const p of theaterPostures) {
            if (p.totalAircraft + p.totalVessels === 0)
                continue;
    ...
    const composite = Math.min(100, Math.round(convergenceScore * convergenceWeight +
...
function identifyTopRisks(convergence, cii, sanctions, radiation) {
    const risks = [];
