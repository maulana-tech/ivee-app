 ...
 */
import { SIMILARITY_THRESHOLD, PREDICTION_SHIFT_THRESHOLD, MARKET_MOVE_THRESHOLD, NEWS_VELOCITY_THRESHOLD, FLOW_PRICE_THRESHOLD, ENERGY_COMMODITY_SYMBOLS, PIPELINE_KEYWORDS, FLOW_DROP_KEYWORDS, TOPIC_KEYWORDS, SUPPRESSED_TRENDING_TERMS, tokenize, jaccardSimilarity, includesKeyword, containsTopicKeyword, findRelatedTopics, generateSignalId, generateDedupeKey, } from '@/utils/analysis-constants';
...
function extractTopics(events) {
    ...
    for (const event of events) {
        ...
        for (const kw of TOPIC_KEYWORDS) {
            if (SUPPRESSED_TRENDING_TERMS.has(kw))
                continue;
            if (!containsTopicKeyword(title, kw))
                continue;
...
function countRelatedTopicMentions(newsTopics, market) {
...
export function detectConvergence(events, getSourceType, isRecentDuplicate, markSignalSeen) {
    const signals = [];
...
export function detectTriangulation(events, getSourceType, isRecentDuplicate, markSignalSeen) {
    const signals = [];
 ...
 */
export function analyzeCorrelationsCore(events, predictions, markets, previousSnapshot, getSourceType, isRecentDuplicate, markSignalSeen) {
    const signals = [];
    const now = Date.now();
    ...
    for (const [topic, velocity] of newsTopics) {
        if (SUPPRESSED_TRENDING_TERMS.has(topic))
            continue;
    ...
    for (const market of markets) {
        if (!ENERGY_COMMODITY_SYMBOLS.has(market.symbol))
            continue;
