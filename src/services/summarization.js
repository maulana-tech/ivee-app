...
import { NewsServiceClient } from '@/generated/client/ivee/news/v1/service_client';
...
const emptySummaryFallback = { summary: '', provider: '', model: '', fallback: true, tokens: 0, error: '', errorType: '', status: 'SUMMARIZE_STATUS_UNSPECIFIED', statusDetail: '' };
const API_PROVIDERS = [
...
async function tryApiProvider(providerDef, headlines, geoContext, lang) {
    if (!isFeatureAvailable(providerDef.featureId))
        return null;
 ...
 */
export async function generateSummary(headlines, onProgress, geoContext, lang = 'en', options) {
