...
function portScatterOffset(hullNumber, index) {
    let hash = 0;
    const str = hullNumber || String(index);
    for (let i = 0; i < str.length; i++) {
        hash = ((hash << 5) - hash) + str.charCodeAt(i);
...
function resolvePortCoords(homePort, hullNumber) {
    ...
    if (hullNumber) {
        ...
        if (portName) {
            ...
            if (coords)
                return { ...coords, portName };
        }
    }
    return undefined;
}
export function mergeUSNIWithAIS(aisVessels, usniReport, aisClusters = []) {
...
export function getUSNIFleetStatus() {
