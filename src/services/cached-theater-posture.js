...
export function toPostureData(resp) {
...
export async function fetchCachedTheaterPosture(signal) {
    ...
    const result = await withCallerAbort(breaker.execute(async () => {
        ...
        saveToStorage(data);
        return data;
    }, emptyFallback(), { shouldCache: (r) => r.postures.length > 0 }), signal);
    if (!result || !Array.isArray(result.postures) || result.postures.length === 0) {
        return null;
    }
    return result;
}
export function getCachedPosture() {
    return breaker.getCached();
}
export function hasCachedPosture() {
