...
export function getThreatLabel(level) {
...
export function classifyByKeyword(title, variant = 'full') {
...
import { IntelligenceServiceClient, ApiError, } from '@/generated/client/ivee/intelligence/v1/service_client';
...
function classifyWithAIUncached(title, variant) {
...
export function classifyWithAI(title, variant) {
    const cacheKey = title.trim().toLowerCase().replace(/\s+/g, ' ');
    return classifyBreaker.execute(() => classifyWithAIUncached(title, variant), null, { cacheKey, shouldCache: (result) => result !== null });
}
