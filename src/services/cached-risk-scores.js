...
export function toRiskScores(resp) {
    return {
...
function isValidCiiEntry(e) {
    if (!e || typeof e !== 'object')
        return false;
    const o = e;
    return typeof o.code === 'string' && Number.isFinite(o.score) && VALID_LEVELS.has(o.level);
}
...
export async function fetchCachedRiskScores(signal) {
    ...
    const result = await withCallerAbort(breaker.execute(async () => {
        const resp = await client.getRiskScores({ region: '' });
        const data = toRiskScores(resp);
        saveToStorage(data);
        setHasCachedScores(true);
        return data;
    }, emptyFallback(), { shouldCache: (r) => r.cii.length > 0 }), signal);
...
export function hasCachedScores() {
    return breaker.getCached() !== null;
}
export function toCountryScore(cached) {
    return {
        code: cached.code,
        name: cached.name,
        score: cached.score,
