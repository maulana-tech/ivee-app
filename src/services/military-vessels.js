...
import { KNOWN_NAVAL_VESSELS, MILITARY_VESSEL_PATTERNS, getNearbyHotspot, MILITARY_HOTSPOTS, } from '@/config/military';
import { registerAisCallback, unregisterAisCallback, isAisConfigured, initAisStream, } from './maritime';
...
const breaker = createCircuitBreaker({
    ...
    revivePersistedData: (data) => ({
        ...
        vessels: data.vessels.map((v) => ({
            ...v,
            lastAisUpdate: v.lastAisUpdate instanceof Date ? v.lastAisUpdate : new Date(v.lastAisUpdate),
        })),
    }),
});
...
export function stopVesselHistoryCleanup() {
 ...
 */
export function initMilitaryVesselStream() {
    if (isTracking)
        return;
 ...
 */
export function disconnectMilitaryVesselStream() {
    if (!isTracking)
        return;
 ...
 */
export async function fetchMilitaryVessels() {
 ...
 */
export function getMilitaryVesselsStatus() {
 ...
 */
export function getVesselsNearLocation(lat, lon, radiusDeg = 2) {
    const result = [];
