...
import { INTEL_HOTSPOTS, CONFLICT_ZONES, MILITARY_BASES, UNDERSEA_CABLES, NUCLEAR_FACILITIES, AI_DATA_CENTERS, PIPELINES, } from '@/config';
...
function detectAssetTypes(titles) {
    ...
    const types = Object.entries(ASSET_KEYWORDS)
        .filter(([, keywords]) => tokenized.some(tokens => keywords.some(keyword => matchKeyword(tokens, keyword))))
        .map(([type]) => type);
    return types;
}
...
export function getClusterAssetContext(cluster) {
...
export function getNearbyInfrastructure(lat, lon, types) {
 ...
 */
export function getCountryInfrastructure(lat, lon, countryCode, types) {
    const results = [];
