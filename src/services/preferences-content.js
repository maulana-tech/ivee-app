...
import { getMapProvider, setMapProvider, MAP_PROVIDER_OPTIONS, MAP_THEME_OPTIONS, getMapTheme, setMapTheme } from '@/config/basemap';
import { getLiveStreamsAlwaysOn, setLiveStreamsAlwaysOn } from '@/services/live-stream-settings';
import { getGlobeVisualPreset, setGlobeVisualPreset, GLOBE_VISUAL_PRESET_OPTIONS } from '@/services/globe-render-settings';
import { getThemePreference, setThemePreference } from '@/utils/theme-manager';
import { getFontFamily, setFontFamily } from '@/services/font-settings';
import { escapeHtml } from '@/utils/sanitize';
import { renderSVG } from 'uqr';
import { trackLanguageChange } from '@/services/analytics';
import { exportSettings, importSettings } from '@/utils/settings-persistence';
import { getChannelsData, createPairingToken, setEmailChannel, startSlackOAuth, startDiscordOAuth, deleteChannel, saveAlertRules, setQuietHours, setDigestSettings, } from '@/services/notification-channels';
...
import { loadFrameworkLibrary, saveImportedFramework, deleteImportedFramework, renameImportedFramework, getActiveFrameworkForPanel, } from '@/services/analysis-framework-store';
const DESKTOP_RELEASES_URL = 'https://github.com/koala73/ivee/releases';
function toggleRowHtml(id, label, desc, checked) {
    return `
...
function renderMapThemeDropdown(container, provider) {
    const select = container.querySelector('#us-map-theme');
    if (!select)
        return;
...
function updateAiStatus(container) {
...
export function renderPreferences(host) {
    const settings = getAiFlowSettings();
    ...
    html += `</select>`;
    html += toggleRowHtml('us-live-streams-always-on', t('components.insights.streamAlwaysOnLabel'), t('components.insights.streamAlwaysOnDesc'), getLiveStreamsAlwaysOn());
    html += `</div></details>`;
    // ── Panels group ──
    html += `<details class="wm-pref-group">`;
        ...
        attach(container) {
            ...
            container.addEventListener('change', (e) => {
                ...
                if (target.id === 'usImportInput') {
                    ...
                    if (!file)
                        return;
                    importSettings(file).then((result) => {
                ...
                if (target.id === 'us-stream-quality') {
                    setStreamQuality(target.value);
                    return;
                }
                if (target.id === 'us-globe-visual-preset') {
                    setGlobeVisualPreset(target.value);
                    return;
                }
                ...
                if (target.id === 'us-cloud') {
                    ...
                    const warn = container.querySelector('.ai-flow-toggle-warn');
                    if (warn)
            ...
            container.addEventListener('click', (e) => {
                ...
                if (target.closest('#usImportBtn')) {
                    container.querySelector('#usImportInput')?.click();
                    return;
                }
                // ── Framework settings handlers ──
                if (target.closest('#fwImportBtn')) {
                    const backdrop = container.querySelector('#fwImportModalBackdrop');
                    if (backdrop)
                ...
                const tab = target.closest('[data-fw-tab]');
                if (tab?.dataset.fwTab) {
                    const tabId = tab.dataset.fwTab;
                    container.querySelectorAll('.fw-import-tab').forEach(el => el.classList.toggle('active', el.dataset.fwTab === tabId));
                ...
                if (target.closest('#fwFetchBtn')) {
                    const urlInput = container.querySelector('#fwAgentskillsUrl');
                    const errEl = container.querySelector('#fwAgentskillsError');
                    const preview = container.querySelector('#fwAgentskillsPreview');
                    if (!urlInput)
                        return;
                    ...
                    const fetchBtn = container.querySelector('#fwFetchBtn');
                    if (fetchBtn)
                    ...
                    fetch('/api/skills/fetch-agentskills', {
                        ...
                        const nameEl = container.querySelector('#fwPreviewName');
                        const descEl = container.querySelector('#fwPreviewDesc');
                        if (nameEl)
                            nameEl.textContent = data.name ?? 'Unnamed skill';
                        if (descEl)
                            descEl.textContent = data.instructions.slice(0, 200) + (data.instructions.length > 200 ? '…' : '');
                        if (preview) {
                ...
                if (target.closest('#fwAgentskillsSaveBtn')) {
                    ...
                    try {
                        ...
                        showImportError(errEl, err.message);
                    }
                    return;
                }
                if (target.closest('#fwJsonSaveBtn')) {
                    const textarea = container.querySelector('#fwJsonInput');
                    const errEl = container.querySelector('#fwJsonError');
                    if (!textarea)
                        return;
                    hideImportError(errEl);
                    let parsed;
                ...
                const deleteBtn = target.closest('.fw-delete-btn');
                if (deleteBtn?.dataset.fwId) {
                    deleteImportedFramework(deleteBtn.dataset.fwId);
                    refreshFrameworkLibrary(container);
                    return;
                }
                const renameBtn = target.closest('.fw-rename-btn');
            ...
            if (!host.isDesktopApp && host.isSignedIn && getEntitlementState() !== null && !hasTier(1)) {
                const upgradeBtn = container.querySelector('#usNotifUpgradeBtn');
                if (upgradeBtn) {
            ...
            if (!host.isDesktopApp && host.isSignedIn && (getEntitlementState() === null || hasTier(1))) {
                ...
                const CHANNEL_LABELS = { telegram: 'Telegram', email: 'Email', slack: 'Slack', discord: 'Discord' };
                function renderChannelRow(channel, type) {
              ...
              <div class="us-notif-ch-actions">
                ...
                function renderNotifContent(data) {
                    const channelTypes = ['telegram', 'email', 'slack', 'discord'];
            ...
            <select class="unified-settings-select" id="usSharedTimezone" style="width:100%">${makeTzOptions(sharedTz)}</select>`;
                ...
                function reloadNotifSection() {
                    const loadingEl = container.querySelector('#usNotifLoading');
                    const contentEl = container.querySelector('#usNotifContent');
                    ...
                    getChannelsData().then((data) => {
                        ...
                        if (loadingEl)
                            loadingEl.textContent = 'Failed to load notification settings.';
                    });
                }
                reloadNotifSection();
                ...
                function saveRuleWithNewChannel(newChannel) {
                    const enabledEl = container.querySelector('#usNotifEnabled');
                    const sensitivityEl = container.querySelector('#usNotifSensitivity');
                    if (!enabledEl)
                        return;
                    const enabled = enabledEl.checked;
                    const sensitivity = (sensitivityEl?.value ?? 'all');
                    const existing = Array.from(container.querySelectorAll('[data-channel-type]'))
                        .filter(el => el.classList.contains('us-notif-ch-on'))
                        .map(el => el.dataset.channelType);
                ...
                let slackOAuthPopup = null;
                let discordOAuthPopup = null;
                let alertRuleDebounceTimer = null;
                let qhDebounceTimer = null;
                let digestDebounceTimer = null;
                ...
                const saveQuietHours = () => {
                    ...
                    qhDebounceTimer = setTimeout(() => {
                        const enabledEl = container.querySelector('#usQhEnabled');
                        const startEl = container.querySelector('#usQhStart');
                        const endEl = container.querySelector('#usQhEnd');
                        const tzEl = container.querySelector('#usSharedTimezone');
                        const overrideEl = container.querySelector('#usQhOverride');
                        void setQuietHours({
                            ...
                            quietHoursOverride: (overrideEl?.value ?? 'critical_only'),
                        });
                    }, 800);
                };
                const saveDigestSettings = () => {
                    if (digestDebounceTimer)
                        clearTimeout(digestDebounceTimer);
                    digestDebounceTimer = setTimeout(() => {
                        const modeEl = container.querySelector('#usDigestMode');
                        const hourEl = container.querySelector('#usDigestHour');
                        const tzEl = container.querySelector('#usSharedTimezone');
                        void setDigestSettings({
                            variant: SITE_VARIANT,
                            digestMode: (modeEl?.value ?? 'realtime'),
                ...
                container.addEventListener('change', (e) => {
                    ...
                    if (target.id === 'usQhEnabled') {
                        const details = container.querySelector('#usQhDetails');
                        if (details)
                            details.style.display = target.checked ? '' : 'none';
                        saveQuietHours();
                        return;
                    }
                    ...
                    if (target.id === 'usDigestMode') {
                        ...
                        const realtimeSection = container.querySelector('#usRealtimeSection');
                        const digestDetails = container.querySelector('#usDigestDetails');
                        if (realtimeSection)
                            realtimeSection.style.display = isRt ? '' : 'none';
                        if (digestDetails)
                            digestDetails.style.display = isRt ? 'none' : '';
                        saveDigestSettings();
                    ...
                    if (target.id === 'usNotifEnabled' || target.id === 'usNotifSensitivity') {
                        ...
                        alertRuleDebounceTimer = setTimeout(() => {
                            ...
                            const sensitivity = (sensitivityEl?.value ?? 'all');
                            const connectedChannelTypes = Array.from(container.querySelectorAll('[data-channel-type]'))
                                .filter(el => el.classList.contains('us-notif-ch-on'))
                                .map(el => el.dataset.channelType);
                            void saveAlertRules({
                                variant: SITE_VARIANT,
                ...
                container.addEventListener('click', (e) => {
                    ...
                    if (target.closest('.us-notif-tg-copy-btn')) {
                        const btn = target.closest('.us-notif-tg-copy-btn');
                    ...
                    const startTelegramPairing = (rowEl) => {
                        ...
                        createPairingToken().then(({ token, expiresAt }) => {
                            if (signal.aborted)
                                return;
                            const botUsername = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_TELEGRAM_BOT_USERNAME) ?? 'IveeBot';
              ...
              `;
                  ...
                  `;
                    ...
                    if (target.closest('#usConnectTelegram') || target.closest('.us-notif-tg-regen')) {
                        const rowEl = target.closest('.us-notif-ch-row');
                        if (!rowEl)
                            return;
                    ...
                    if (target.closest('#usConnectSlack')) {
                        const btn = target.closest('#usConnectSlack');
                        ...
                        startSlackOAuth().then((oauthUrl) => {
                            ...
                            if (!popup) {
                                ...
                                const rowEl = btn?.closest('[data-channel-type="slack"]');
                                if (rowEl) {
                                    rowEl.querySelector('.us-notif-error')?.remove();
                    ...
                    if (target.closest('#usConnectDiscord')) {
                        const btn = target.closest('#usConnectDiscord');
                        ...
                        startDiscordOAuth().then((oauthUrl) => {
                            ...
                            if (!popup) {
                                ...
                                const rowEl = btn?.closest('[data-channel-type="discord"]');
                                if (rowEl) {
                                    rowEl.querySelector('.us-notif-error')?.remove();
                                    rowEl.insertAdjacentHTML('beforeend', '<span class="us-notif-error">Popup blocked — please allow popups for this site, then try again.</span>');
                                }
                    ...
                    const disconnectBtn = target.closest('.us-notif-disconnect[data-channel]');
                    if (disconnectBtn?.dataset.channel) {
                        const channelType = disconnectBtn.dataset.channel;
                ...
                const onMessage = (e) => {
                    ...
                    if (!trustedOrigin || (!fromSlack && !fromDiscord))
                        return;
                    if (e.data?.type === 'wm:slack_connected') {
                        ...
                        const rowEl = container.querySelector('[data-channel-type="slack"]');
                        if (rowEl) {
                            rowEl.querySelector('.us-notif-error')?.remove();
                            ...
                            const btn = rowEl.querySelector('#usConnectSlack');
                            if (btn)
                                btn.textContent = 'Add to Slack';
                        }
                    }
                        ...
                        const rowEl = container.querySelector('[data-channel-type="discord"]');
                        if (rowEl) {
                            rowEl.querySelector('.us-notif-error')?.remove();
                            ...
                            const btn = rowEl.querySelector('#usConnectDiscord');
                            if (btn)
                                btn.textContent = 'Connect Discord';
                        }
                    }
...
function renderFrameworkLibraryHtml() {
...
function refreshFrameworkLibrary(container) {
    const list = container.querySelector('#fwLibraryList');
    if (list)
        list.innerHTML = renderFrameworkLibraryHtml();
}
...
function hideImportError(el) {
    if (!el)
        return;
    el.textContent = '';
    el.style.display = 'none';
}
function showToast(container, msg, success) {
