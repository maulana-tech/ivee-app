...
export function getCachedMarketImplications() {
    return cache.get('')?.data ?? null;
}
export async function fetchMarketImplications(frameworkId = '') {
    const now = Date.now();
    const cached = cache.get(frameworkId);
    if (cached && !cached.data.degraded && now - cached.cachedAt < CACHE_TTL)
        return cached.data;
    if (!frameworkId) {
        const hydrated = getHydratedData('marketImplications');
        if (hydrated?.cards && Array.isArray(hydrated.cards) && hydrated.cards.length > 0 && !hydrated.degraded) {
            const data = {
                cards: hydrated.cards.map(c => normalizeCard(c)),
                degraded: false,
    ...
    try {
        ...
        if (!Array.isArray(raw.cards))
            return cached?.data ?? null;
        const data = {
            cards: raw.cards.map(c => normalizeCard(c)),
