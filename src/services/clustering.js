...
export function clusterNews(items) {
    return clusterNewsCore(items, getSourceTier);
}
/**
 * Hybrid clustering: Jaccard first, then semantic refinement if ML available
 */
export async function clusterNewsHybrid(items) {
    // Step 1: Fast Jaccard clustering
    const jaccardClusters = clusterNewsCore(items, getSourceTier);
    ...
    try {
        ...
        const semanticGroups = await mlWorker.clusterBySemanticSimilarity(clusterTexts, ML_THRESHOLDS.semanticClusterThreshold);
