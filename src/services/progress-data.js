...
function buildDataSet(indicator, data) {
    if (data.length === 0)
...
function resolveFromSeeds(seedMap) {
...
async function fetchProgressDataFresh() {
    ...
    const hydrated = getHydratedData('progressData');
    if (hydrated?.length)
        return resolveFromSeeds(buildSeedMap(hydrated));
