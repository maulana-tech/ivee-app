...
import { REFRESH_INTERVALS, DEFAULT_PANELS, DEFAULT_MAP_LAYERS, MOBILE_DEFAULT_MAP_LAYERS, STORAGE_KEYS, SITE_VARIANT, ALL_PANELS, VARIANT_DEFAULTS, getEffectivePanelConfig, FREE_MAX_PANELS, FREE_MAX_SOURCES, } from '@/config';
...
import { fetchBootstrapData, getBootstrapHydrationState, markBootstrapAsLive } from '@/services/bootstrap';
...
import { resolveUserRegion, resolvePreciseUserCoordinates } from '@/utils/user-location';
...
import { CorrelationEngine, militaryAdapter, escalationAdapter, economicAdapter, disasterAdapter, } from '@/services/correlation-engine';
...
export class App {
    ...
    getCachedBootstrapUpdatedAt() {
        ...
        if (cachedTierTimestamps.length === 0)
            return null;
    ...
    async primeVisiblePanelData(forceAll = false) {
        const tasks = [];
        const primeTask = (key, task) => {
        ...
        const shouldPrime = (id) => forceAll || this.isPanelNearViewport(id);
        const shouldPrimeAny = (ids) => forceAll || this.isAnyPanelNearViewport(ids);
        if (shouldPrime('yield-curve')) {
            const panel = this.state.panels['yield-curve'];
            if (panel)
                primeTask('yield-curve', () => panel.fetchData());
        }
        if (shouldPrime('earnings-calendar')) {
            const panel = this.state.panels['earnings-calendar'];
            if (panel)
                primeTask('earnings-calendar', () => panel.fetchData());
        }
        if (shouldPrime('economic-calendar')) {
            const panel = this.state.panels['economic-calendar'];
            if (panel)
                primeTask('economic-calendar', () => panel.fetchData());
        }
        if (shouldPrime('cot-positioning')) {
            const panel = this.state.panels['cot-positioning'];
            if (panel)
                primeTask('cot-positioning', () => panel.fetchData());
        }
    ...
    constructor(containerId) {
        ...
        Object.defineProperty(this, "pendingDeepLinkCountry", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "pendingDeepLinkExpanded", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        });
        Object.defineProperty(this, "pendingDeepLinkStoryCode", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "panelLayout", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "searchManager", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "countryIntel", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "refreshScheduler", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "desktopUpdater", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "unsubAiFlow", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "unsubFreeTier", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "visiblePanelPrimed", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Set()
        });
        Object.defineProperty(this, "visiblePanelPrimeRaf", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "bootstrapHydrationState", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: getBootstrapHydrationState()
        });
        Object.defineProperty(this, "cachedModeBannerEl", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "handleViewportPrime", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: () => {
        ...
        Object.defineProperty(this, "handleConnectivityChange", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: () => {
        ...
        const monitors = loadFromStorage(STORAGE_KEYS.monitors, []);
        ...
        const isDynamicPanel = (k) => k === 'runtime-config' || k.startsWith('cw-') || k.startsWith('mcp-');
        ...
        if (storedVariant !== currentVariant) {
            ...
            mapLayers = normalizeExclusiveChoropleths(sanitizeLayersForVariant({ ...defaultLayers }, currentVariant), null);
            // Load existing panel prefs (if any), disable panels not belonging to the new variant
            panelSettings = loadFromStorage(STORAGE_KEYS.panels, {});
            ...
            mapLayers = normalizeExclusiveChoropleths(sanitizeLayersForVariant(loadFromStorage(STORAGE_KEYS.mapLayers, defaultLayers), currentVariant), null);
            panelSettings = loadFromStorage(STORAGE_KEYS.panels, DEFAULT_PANELS);
            ...
            if (!localStorage.getItem(PANEL_KEY_RENAMES_MIGRATION_KEY)) {
                ...
                const keyRenames = [
                    ...
                    ...(SITE_VARIANT === 'finance' ? [['regulation', 'fin-regulation']] : []),
                ];
                ...
                for (const [legacyKey, nextKey] of keyRenames) {
                    if (!panelSettings[legacyKey] || panelSettings[nextKey])
                        continue;
                ...
                if (!localStorage.getItem(TECH_INSIGHTS_MIGRATION_KEY)) {
                    ...
                    if (savedOrder) {
                        try {
                            ...
                            const newOrder = [];
                            if (order.includes('live-news'))
                                newOrder.push('live-news');
                            if (order.includes('insights'))
                                newOrder.push('insights');
                            newOrder.push(...filtered);
                            localStorage.setItem(PANEL_ORDER_KEY, JSON.stringify(newOrder));
        ...
        if (!localStorage.getItem(PANEL_PRUNE_KEY)) {
            ...
            for (const orderKey of [PANEL_ORDER_KEY, PANEL_ORDER_KEY + '-bottom-set', PANEL_ORDER_KEY + '-bottom']) {
                try {
                    ...
                    if (!Array.isArray(arr))
                        continue;
                    const filtered = arr.filter((k) => validKeys.has(k));
        ...
        if (initialUrlState.layers) {
            mapLayers = normalizeExclusiveChoropleths(sanitizeLayersForVariant(initialUrlState.layers, currentVariant), null);
        ...
        if (currentVariant === 'full') {
            ...
            if (userLang !== 'en' && !localStorage.getItem(localeKey)) {
                ...
                if (boosted.size > 0) {
                    const current = loadFromStorage(STORAGE_KEYS.disabledFeeds, []);
        ...
        const disabledSources = new Set(loadFromStorage(STORAGE_KEYS.disabledFeeds, []));
    ...
    async init() {
            ...
            if (userId !== null && userId !== _prevUserId) {
                ...
                if (anonId) {
                    void (async () => {
                        ...
                        const ready = await waitForConvexAuth(10000);
                        if (!ready) {
        ...
        const geoCoordsPromise = this.state.isMobile && this.state.initialUrlState?.lat === undefined && this.state.initialUrlState?.lon === undefined
        ...
        if (!this.state.isMobile) {
            ...
            this.state.findingsBadge.setOnSignalClick((signal) => {
                if (this.state.countryBriefPage?.isVisible())
                    return;
                if (localStorage.getItem('wm-settings-open') === '1')
                    return;
    ...
    enforceFreeTierLimits() {
        if (isProUser())
            return;
        // --- Panel limit ---
        const panelSettings = loadFromStorage(STORAGE_KEYS.panels, {});
    ...
    setupRefreshIntervals() {
        ...
        if (SITE_VARIANT === 'finance') {
            this.refreshScheduler.scheduleRefresh('stock-analysis', () => this.dataLoader.loadStockAnalysis(), REFRESH_INTERVALS.stockAnalysis, () => hasPremiumAccess() && this.isPanelNearViewport('stock-analysis'));
            this.refreshScheduler.scheduleRefresh('daily-market-brief', () => this.dataLoader.loadDailyMarketBrief(), REFRESH_INTERVALS.dailyMarketBrief, () => hasPremiumAccess() && this.isPanelNearViewport('daily-market-brief'));
            this.refreshScheduler.scheduleRefresh('stock-backtest', () => this.dataLoader.loadStockBacktest(), REFRESH_INTERVALS.stockBacktest, () => hasPremiumAccess() && this.isPanelNearViewport('stock-backtest'));
            this.refreshScheduler.scheduleRefresh('market-implications', () => this.dataLoader.loadMarketImplications(), REFRESH_INTERVALS.marketImplications, () => hasPremiumAccess() && this.isPanelNearViewport('market-implications'));
        }
        ...
        this.refreshScheduler.scheduleRefresh('cross-source-signals', () => this.dataLoader.loadCrossSourceSignals(), REFRESH_INTERVALS.crossSourceSignals, () => this.isPanelNearViewport('cross-source-signals'));
        ...
        if (SITE_VARIANT === 'full') {
            this.refreshScheduler.scheduleRefresh('intelligence', () => {
                ...
                if (military)
                    this.state.intelligenceCache.military = military;
                if (iranEvents)
                    this.state.intelligenceCache.iranEvents = iranEvents;
