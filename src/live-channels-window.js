import { loadChannelsFromStorage, saveChannelsToStorage, BUILTIN_IDS, getDefaultLiveChannels, getFilteredOptionalChannels, getFilteredChannelRegions, } from '@/components/LiveNewsPanel';
...
function customChannelIdFromHandle(handle) {
...
export async function initLiveChannelsWindow(containerEl) {
    ...
    function applyOrderFromDom(listEl) {
        const rows = listEl.querySelectorAll('.live-news-manage-row');
        const ids = Array.from(rows).map((el) => el.dataset.channelId).filter((id) => !!id);
        const map = new Map(channels.map((c) => [c.id, c]));
        channels = ids.map((id) => map.get(id)).filter((c) => !!c);
    ...
    function setupListDnD(listEl) {
        ...
        listEl.addEventListener('mousedown', (e) => {
            ...
            if (target.closest('input, button, textarea, select'))
                return;
            const row = target.closest('.live-news-manage-row');
        ...
        document.addEventListener('mousemove', (e) => {
            if (!dragging)
                return;
            if (!dragStarted) {
                if (Math.abs(e.clientY - startY) < THRESHOLD)
                    return;
                dragStarted = true;
                dragging.classList.add('live-news-manage-row-dragging');
            }
            const target = document.elementFromPoint(e.clientX, e.clientY)?.closest('.live-news-manage-row');
        ...
        for (const ch of channels) {
            ...
            if (isCustom) {
                row.addEventListener('click', (e) => {
                    if (suppressRowClick || row.classList.contains('live-news-manage-row-dragging'))
                        return;
                    if (e.target.closest('input, button, textarea, select, .live-news-manage-row-remove-x'))
                        return;
                    e.preventDefault();
    ...
    function getMissingDefaultChannels() {
    ...
    function updateRestoreButton() {
        ...
        btn.style.display = missing.length > 0 ? '' : 'none';
    }
    /**
     ...
     */
    function applyEditFormToChannels(currentCh, formRow, isCustom, displayName) {
        const idx = channels.findIndex((c) => c.id === currentCh.id);
        if (idx === -1)
            return null;
        if (isCustom) {
            const handleRaw = formRow.querySelector('.live-news-manage-edit-handle')?.value?.trim();
            if (handleRaw) {
    ...
    function showEditForm(row, ch, listEl) {
        const isCustom = !BUILTIN_IDS.has(ch.id);
        row.innerHTML = '';
    ...
    function renderAvailableChannels(listEl) {
        ...
        if (term) {
            ...
            if (!activeHasMatch) {
                const firstMatch = filteredRegions.find(r => r.channelIds.some(id => {
                    const ch = optionalChannelMap.get(id);
                    return ch && (ch.name.toLowerCase().includes(term) || ch.handle?.toLowerCase().includes(term));
                }));
                if (firstMatch)
                    activeRegionTab = firstMatch.key;
            }
        }
        // Render tab buttons
        tabBar.innerHTML = '';
    ...
    function createCard(ch, isAdded, listEl) {
  ...
  `;
    ...
    const addBtn = document.getElementById('liveChannelsAddBtn');
    addBtn?.addEventListener('click', async () => {
        const handleInput = document.getElementById('liveChannelsHandle');
        const hlsInput = document.getElementById('liveChannelsHlsUrl');
        const nameInput = document.getElementById('liveChannelsName');
        ...
        if (hlsInput)
            hlsInput.classList.remove('invalid');
    ...
    const searchInput = document.getElementById('liveChannelsSearch');
