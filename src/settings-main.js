...
import { SETTINGS_CATEGORIES, HUMAN_LABELS, SIGNUP_URLS, PLAINTEXT_KEYS, MASKED_SENTINEL, } from '@/services/settings-constants';
import { fetchOllamaModels } from '@/services/ollama-models';
import { RUNTIME_FEATURES, getEffectiveSecrets, getRuntimeConfigSnapshot, getSecretState, isFeatureAvailable, isFeatureEnabled, setFeatureToggle, setSecretValue, validateSecret, loadDesktopSecrets, } from '@/services/runtime-config';
import { getApiBaseUrl, isDesktopRuntime, resolveLocalApiPort, startSmartPollLoop } from '@/services/runtime';
...
async function invokeDesktopAction(command, successLabel) {
    const result = await tryInvokeTauri(command);
    if (result) {
...
function closeSettingsWindow() {
    void tryInvokeTauri('close_settings_window').then(() => { }, () => window.close());
}
function getSidecarBase() {
    return getApiBaseUrl() || '';
}
let _diagToken = null;
async function diagFetch(path, init) {
    if (!_diagToken) {
        try {
            _diagToken = await tryInvokeTauri('get_local_api_token');
        }
        catch { /* token unavailable */ }
    }
    const headers = new Headers(init?.headers);
    if (_diagToken)
        headers.set('Authorization', `Bearer ${_diagToken}`);
...
// ── Sidebar ──
function getFeatureStatusCounts(cat) {
    let ready = 0;
    for (const fid of cat.features) {
        if (isFeatureAvailable(fid))
            ready++;
    }
...
function initOverviewListeners(area) {
    area.querySelector('[data-wm-toggle]')?.addEventListener('click', () => {
        const input = area.querySelector('[data-wm-key-input]');
        if (input)
            input.type = input.type === 'password' ? 'text' : 'password';
    });
    area.querySelector('[data-wm-key-input]')?.addEventListener('input', (e) => {
    ...
    area.querySelector('[data-wm-open-pro]')?.addEventListener('click', () => {
        const url = 'https://ivee.app/pro';
        void invokeTauri('open_url', { url }).catch(() => window.open(url, '_blank'));
    });
    area.querySelectorAll('.settings-ov-cat[data-section]').forEach(btn => {
...
// ── Feature sections ──
function renderFeatureSection(area, cat) {
    ...
    const featureCards = features.map(feature => {
        ...
        const allStaged = !available && effectiveSecrets.every(k => getSecretState(k).valid || (settingsManager.hasPending(k) && settingsManager.getValidationState(k).validated !== false));
...
function renderSecretInput(key, _featureId) {
...
function initFeatureSectionListeners(area) {
    area.querySelectorAll('[data-feat-toggle-expand]').forEach(header => {
        header.addEventListener('click', (e) => {
            if (e.target.closest('[data-click-stop]'))
                return;
            const card = header.closest('.settings-feat');
            card?.classList.toggle('expanded');
        });
    });
    area.querySelectorAll('input[data-toggle]').forEach(input => {
    ...
    area.querySelectorAll('input[data-secret]').forEach(input => {
        ...
        input.addEventListener('blur', () => {
            ...
            updateFeatureCardStatus(input.dataset.feature);
            if (key === 'OLLAMA_API_URL' && result.valid) {
                const modelSelect = area.querySelector('select[data-model-select]');
                if (modelSelect)
    ...
    area.querySelectorAll('a[data-signup-url]').forEach(link => {
...
function updateFeatureCardStatus(featureId) {
    const card = document.querySelector(`.settings-feat[data-feature-id="${featureId}"]`);
    if (!card)
        return;
...
async function loadOllamaModelsIntoSelect(select) {
    ...
    select.innerHTML = options + models.map(name => `<option value="${escapeHtml(name)}" ${name === currentModel ? 'selected' : ''}>${escapeHtml(name)}</option>`).join('');
}
// ── Debug section ──
function renderDebug(area) {
    area.innerHTML = `
    <div class="settings-section-header">
      <h2>Debug &amp; Logs</h2>
    </div>
  ...
  `;
    ...
    const importInput = area.querySelector('#importSettingsInput');
...
function initDiagnostics() {
    const verboseToggle = document.getElementById('verboseApiLog');
    const fetchDebugToggle = document.getElementById('fetchDebugLog');
    const autoRefreshToggle = document.getElementById('autoRefreshLog');
    ...
    async function syncVerboseState() {
        if (!verboseToggle)
            return;
        try {
    ...
    async function refreshTrafficLog() {
        ...
        try {
            ...
            if (trafficCount)
                trafficCount.textContent = `(${entries.length})`;
            if (entries.length === 0) {
...
async function initSettingsWindow() {
    ...
    document.getElementById('okBtn')?.addEventListener('click', () => {
        void (async () => {
            try {
                const wmKeyInput = document.querySelector('[data-wm-key-input]');
                ...
                if (contentArea)
                    settingsManager.captureUnsavedInputs(contentArea);
