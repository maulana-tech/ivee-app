...
test.describe('desktop runtime routing guardrails', () => {
    ...
    const result = await page.evaluate(async () => {
      ...
      return {
        ...
        webHost: runtime.detectDesktopRuntime({
          ...
          userAgent: 'Mozilla/5.0',
          locationProtocol: 'https:',
          locationHost: 'ivee.app',
          locationOrigin: 'https://ivee.app',
        }),
      };
    });
  ...
  test('runtime fetch patch falls back to cloud for local failures', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('ivee.app/api/fred-data')) {
        ...
        if (url.includes('ivee.app/api/stablecoin-markets')) {
    ...
    expect(result.calls.some((url) => url.includes('ivee.app/api/fred-data'))).toBe(true);
    expect(result.calls.some((url) => url.includes('127.0.0.1:46123/api/stablecoin-markets'))).toBe(true);
    expect(result.calls.some((url) => url.includes('ivee.app/api/stablecoin-markets'))).toBe(true);
  });
  ...
  test('runtime fetch patch never sends local-only endpoints to cloud', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('ivee.app/api/local-env-update')) {
          return responseJson({ leaked: true }, 200);
        }
        if (url.includes('ivee.app/api/local-validate-secret')) {
          return responseJson({ leaked: true }, 200);
        }
    ...
    expect(result.calls.some((url) => url.includes('ivee.app/api/local-env-update'))).toBe(false);
    expect(result.calls.some((url) => url.includes('ivee.app/api/local-validate-secret'))).toBe(false);
  });
  ...
  test('update badge picks architecture-correct desktop download url', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const previousTauri = globalWindow.__TAURI__;
      const releaseUrl = 'https://github.com/koala73/ivee/releases/latest';
    ...
    expect(result.macArm).toBe('https://ivee.app/api/download?platform=macos-arm64&variant=full');
    expect(result.windowsX64).toBe('https://ivee.app/api/download?platform=windows-exe&variant=full');
    expect(result.linuxFallback).toBe('https://github.com/koala73/ivee/releases/latest');
  });
  ...
  test('cloud fallback blocked without Ivee API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const cloudCalls = calls.filter(u => u.includes('ivee.app'));
  ...
  test('cloud fallback allowed with valid Ivee API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('ivee.app') && init?.headers) {
          const h = new Headers(init.headers);
          const wmKey = h.get('X-Ivee-Key');
          if (wmKey) capturedHeaders['X-Ivee-Key'] = wmKey;
        }

        if (url.includes('127.0.0.1:46123/api/market/v1/test')) {
          throw new Error('ECONNREFUSED');
        }
        if (url.includes('ivee.app/api/market/v1/test')) {
      ...
      try {
        ...
        return {
          ...
          cloudCalls: calls.filter(u => u.includes('ivee.app')).length,
          wmKeyHeader: capturedHeaders['X-Ivee-Key'] || null,
        };
      } finally {
        window.fetch = originalFetch;
        delete globalWindow.__wmFetchPatched;
