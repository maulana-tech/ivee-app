...
test.describe('theme toggle (happy variant)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      if (!sessionStorage.getItem('__test_init_done')) {
        localStorage.removeItem('ivee-theme');
        localStorage.removeItem('ivee-variant');
  ...
  test('dark mode persists across page reload', async ({ page }) => {
    ...
    const stored = await page.evaluate(() => localStorage.getItem('ivee-theme'));
  ...
  test('no FOUC: data-theme is set before main CSS loads', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      localStorage.setItem('ivee-theme', 'dark');
      localStorage.setItem('ivee-variant', 'happy');
    });
